...
import com.hotent.makshi.dao.summary.SummaryCompanyVipDao;
import com.hotent.makshi.model.summary.SummaryCompanyVip;

@Service
public class SummaryCompanyVipService extends BaseService<SummaryCompanyVip> {
	...
	@Resource
	private SummaryCompanyVipDao summaryCompanyVipDao;

	@Override
	protected IEntityDao getEntityDao() {
		return summaryCompanyVipDao;
	}

	public void save(SummaryCompanyVip summaryCompanyVip) {
		Long id = summaryCompanyVip.getId();
		if (id == null || id == 0) {
			id = UniqueIdUtil.genId();
			summaryCompanyVip.setId(id);
			this.add(summaryCompanyVip);
		} else {
			this.update(summaryCompanyVip);
		}
	}

	public void export(List<SummaryCompanyVip> resultList, String fileName, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			for (int i = 0; i < totalSheet; i++) {
				...
				CellRangeAddress reg = new CellRangeAddress(rowIndex, rowIndex, 0, 12);
	...
	public static void initRecordSheetColumns(HSSFSheet sheet) {
		...
		sheet.setColumnWidth(index++, (40 * 150));
		sheet.setColumnWidth(index++, (40 * 100));
		sheet.setColumnWidth(index++, (40 * 80));
		sheet.setColumnWidth(index++, (40 * 80));
		sheet.setColumnWidth(index++, (40 * 80));
		sheet.setColumnWidth(index++, (40 * 100));
	...
	public static void initRecordSheetColumnsHeads(HSSFRow row, HSSFCellStyle rowtitlecellStyle) {
		int index = 0;
		createCell(row, index++, "发证机构", rowtitlecellStyle);
		createCell(row, index++, "级别", rowtitlecellStyle);
		createCell(row, index++, "证书编号", rowtitlecellStyle);
		createCell(row, index++, "董事长协会职务", rowtitlecellStyle);
		...
		createCell(row, index++, "会费缴纳年度", rowtitlecellStyle);
		createCell(row, index++, "所属部门", rowtitlecellStyle);
		createCell(row, index++, "协会联系人", rowtitlecellStyle);
		createCell(row, index++, "协会联系电话", rowtitlecellStyle);
		createCell(row, index++, "备注", rowtitlecellStyle);
	}

	public static int addRecordContentToExcel(List<SummaryCompanyVip> resultList, HSSFRow row, int rowIndex, HSSFSheet sheet, HSSFCellStyle titlecellStyle, HSSFCellStyle evencellStyle,
			HSSFCellStyle oddcellStyle) {
		// 用来存放奇偶数行的临时对象
		HSSFCellStyle tempcellStyle = null;
		for (SummaryCompanyVip d : resultList) {
			row = sheet.createRow(++rowIndex);
			...
			createCell(row, index++, d.getDuties() + "", tempcellStyle);
			...
			createCell(row, index++, d.getVipYears() + "年", tempcellStyle);
			createCell(row, index++, d.getDepartment() + "", tempcellStyle);
			createCell(row, index++, d.getContactser() + "", tempcellStyle);
			createCell(row, index++, d.getContactsPhone() + "", tempcellStyle);
