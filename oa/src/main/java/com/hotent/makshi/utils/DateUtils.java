...
public final class DateUtils {
	...
	public static int getDiffDaysByDay(Date sDate, Date eDate) {
		...
		return (int) ((eCalendar.getTimeInMillis() - sCalendar.getTimeInMillis()) / MILLIS_ONE_DAY);
	...
	public static List<String> getMonthEndDateList(int startDay, int endDay) {
		...
		if (startYear == endYear) {
			for (int m = startMonth; m <= endMonth; m++) {
				...
				if (m < 10)
					monStr = "0" + m;
				String date = startYear + "" + monStr + "" + getDaysOfCurMonth(startYear + "-" + m);
				dateList.add(date);
			}
		} else {
			for (int y = startYear; y <= endYear; y++) {
				if (y == startYear) {
					for (int m = startMonth; m <= 12; m++) {
						...
						if (m < 10)
							monStr = "0" + m;
						String date = y + "" + monStr + "" + getDaysOfCurMonth(y + "-" + m);
						dateList.add(date);
					}
				} else if (y == endYear) {
	...
	public static String transDate(String date) {
		if (date == null || date.trim().equals("null") || date.trim().equals("") || date.trim().equals("--")) {
			return null;
		}
	...
	public static Long getWorkdayTimeInMillisExcWeekendHolidays(Long start, Long end, List<Date> listHolidays) {
		...
		if ((sdate.get(Calendar.YEAR) == edate.get(Calendar.YEAR)) && (sdate.get(Calendar.WEEK_OF_YEAR) == edate.get(Calendar.WEEK_OF_YEAR))
				&& (sdate.get(Calendar.DAY_OF_WEEK) != 1 && sdate.get(Calendar.DAY_OF_WEEK) != 7) && (edate.get(Calendar.DAY_OF_WEEK) != 1 && edate.get(Calendar.DAY_OF_WEEK) != 7)) {
			return new Long(end - start - holidays);
		}
		// 如果两个时间在同一周并且都是周末日期，则直接返回0
		if ((sdate.get(Calendar.YEAR) == edate.get(Calendar.YEAR)) && (sdate.get(Calendar.WEEK_OF_YEAR) == edate.get(Calendar.WEEK_OF_YEAR) - 1)
				&& (sdate.get(Calendar.DAY_OF_WEEK) == 1 || sdate.get(Calendar.DAY_OF_WEEK) == 7) && (edate.get(Calendar.DAY_OF_WEEK) == 1 || edate.get(Calendar.DAY_OF_WEEK) == 7)) {
		...
		long result = (workdays * 24 * 3600000 + calcWorkdayTimeInMillis(sdate, edate, start, end) - holidays);
	...
	public static long validateEndTime(Calendar edate) {
		if (edate.get(Calendar.DAY_OF_WEEK) == 1)// 结束日期是周日,如果结束日期是周六、周末自动修复为这周五18:00
		{
			...
			edate.setTimeInMillis(
					edate.getTime().getTime() + (18 * 3600000 - ((edate.get(Calendar.HOUR_OF_DAY) * 3600000) + (edate.get(Calendar.MINUTE) * 60000) + (edate.get(Calendar.SECOND) * 1000))));
	...
	public static long getHolidaysInMillis(long start, long end, List<Date> listHolidays) {
		Calendar scalendar = Calendar.getInstance();
		Calendar ecalendar = Calendar.getInstance();
		int daysofH = 0;
		try {
	...
	public static String getDefinedMonth(int offmonth, String format) {
	...
	public static String getDefinedYear(int offyear, String format) {
		if (StringUtils.isEmpty(format))
			return null;
		Calendar calendar = Calendar.getInstance();
	...
	/*
	 * public static void main(String[] args) { System.out.println(getDefinedYear(1, "yyyy")); System.out.println(getDefinedYear(0, "yyyy")); System.out.println(getDefinedYear(-1, "yyyy")); }
	 */

	/**
	 * 比较两个日期大小，按天 date1<date2,负数;date1=date2,0;date1>date2,正数;
	 * 
	 * @param date1
	 * @param date2
	 * @return
	 */
	public static int compareDatesInDay(final Date date1, final Date date2) {
		...
		String bg = sdf.format(date1);
		String end = sdf.format(date2);

		int result = bg.compareTo(end);
	...
	public static void main(String[] args) {
		String start = (DateUtils.getDefinedMonth(-5, "yyyy-MM"));
		String end = (DateUtils.getDefinedYear(0, "yyyy-MM"));

		List<String> dates = new ArrayList<String>();
		dates.add(start);
		while (true) {
			String temp = DateUtils.format(DateUtils.addMonth(DateUtils.toDateFromStr(start + "-01 00:00:00"), 1), "yyyy-MM");
			if (temp.equals(end))
				break;
			start = temp;
			dates.add(temp);
