...
public class ProcessServiceImpl implements ProcessService {
	...
	protected Logger logger = LoggerFactory.getLogger(ProcessServiceImpl.class);
	...
	public String doNext() {
		...
		SysUser curUser = sysUserService.getById(Long.valueOf(RequestUtil.getString(request, "userId")));
		// PrintWriter out = response.getWriter();
		String taskId = RequestUtil.getString(request, "taskId");
		TaskEntity task = bpmService.getTask(taskId);
		if (task == null) {
			// resultMessage = new ResultMessage(ResultMessage.Fail, "此任务已经执行了!");
			result.setResultMap(null, ReturnCode.TASK_COMPLETED_ERROR);
		...
		if (BpmDefinition.STATUS_INST_DISABLED.equals(bpmDefinition.getStatus())) {
			result.setResultMap(null, ReturnCode.PROCESSINSTANCE_FORBID);
			return result.getResultMap().toString();
		}
		...
		try {
			...
			if (StringUtil.isNotEmpty(str)) {
				result.setResultMap(null, ReturnCode.EXCEPTION_ERROR.getReturnCode(), str);
			} else {
				String message = ExceptionUtil.getExceptionMessage(ex);
				result.setResultMap(null, ReturnCode.EXCEPTION_ERROR.getReturnCode(), message);
			}
			return result.getResultMap().toString();
		}
		...
		if (!isAdmin) {
			...
			if (!rtn) {
				result.setResultMap(null, ReturnCode.TASK_ASSIGNEE_ERROR);
				return result.getResultMap().toString();
			}
		}
		...
		try {
			...
			if (isSignTask) {
				...
				if (sysOrg != null) {
					...
					isAllowAddSign = bpmNodeSignService.checkNodeSignPrivilege(processRun.getActDefId(), taskEntity.getTaskDefinitionKey(), BpmNodeSignService.BpmNodePrivilegeType.ALLOW_RETROACTIVE,
							userId, orgId);
				}
			}
	...
	public String start() {
		try {
			...
			if (StringUtils.isEmpty(actDefId) && StringUtils.isEmpty(flowkey)) {
				// String errorMesage = genMessage(false, "流程定义ID和流程key必须填写一个!");
				// return errorMesage;
				throw new ApiExcetpion(ReturnCode.PRAMS_IS_NULL, "流程定义ID和流程key必须填写一个!");
			}
			// 启动账户必填。
			if (StringUtils.isEmpty(request.getParameter("account"))) {// 没传用户就报错
				throw new ApiExcetpion(ReturnCode.PRAMS_IS_NULL, "必须传入用户账号(account)");
			}
			startUser = request.getParameter("account");
			ProcessCmd processCmd = new ProcessCmd();
			...
			if (StringUtils.isNotEmpty(request.getParameter("businessKey"))) {
			...
			if (StringUtils.isNotEmpty(request.getParameter("subject"))) {
			...
			String datas2 = RequestUtil.getString(request, "data");
			if (StringUtil.isNotEmpty(datas)) {
				if (StringUtils.isNotEmpty(flowkey) && "qjlc".equals(flowkey)) {
					JSONObject jsonObject = JSONObject.fromObject(datas);
					JSONObject main = (JSONObject) jsonObject.get("main");
					JSONObject field = (JSONObject) main.get("fields");
					String leave_type = (String) field.get("leave_type");
					String startTime=(String)field.get("startTime");
					String endTime=(String)field.get("endTime");
					String leaveStartSolt=(String)field.get("leaveStartSolt");
					String leaveEndSolt=(String)field.get("leaveEndSolt");
					Date startDate;
					Date endDate;
					Long userid = null;
					SysUser byAccount = sysUserService.getByAccount(startUser);
					if(byAccount==null){
						throw new ApiExcetpion(ReturnCode.PRAMS_IS_NULL, "用户不存在!");
					}
					userid=byAccount.getUserId();
					try {
						startDate = DateUtils.parse(startTime,"yyyy-MM-dd");
						endDate = DateUtils.parse(endTime,"yyyy-MM-dd");
					} catch (Exception e) {
						throw new ApiExcetpion(ReturnCode.PRAMS_IS_NULL, "请求参数错误!");
					}
					double leave_days = Double.valueOf((String) field.get("leave_days"));
					if("婚假".equals(leave_type) && leave_days>3){
						throw new ApiExcetpion(ReturnCode.PRAMS_IS_NULL, "婚假请假天数不能大于3天!");
					}
					if("陪产假".equals(leave_type) && leave_days>15){
						throw new ApiExcetpion(ReturnCode.PRAMS_IS_NULL, "陪产假请假天数不能大于15天!");
					}
					if("产假".equals(leave_type) && leave_days>178){
						throw new ApiExcetpion(ReturnCode.PRAMS_IS_NULL, "产假请假天数不能大于178天!");
					}
					if("丧假".equals(leave_type) && leave_days>3){
						throw new ApiExcetpion(ReturnCode.PRAMS_IS_NULL, "丧假请假天数不能大于3天!");
					}
					if("年假".equals(leave_type)){
					      AnuualLeave anuualLeave = anuualLeaveService.calculateAnuualVacation(byAccount.getUserId());
					      if (anuualLeave == null) {
					    	  anuualLeave = new AnuualLeave(0d, 0d, 0d, 0d);
					      }
					      if(anuualLeave.getActualLeaveVacation()-leave_days<0){       
					    	  throw new ApiExcetpion(ReturnCode.PRAMS_ERROR, "请假天数不能大于年假天数！");
					      }
					}
					Map<Integer, String> validVaction = VactionUtils.validVaction(startDate, endDate, Integer.valueOf(leaveStartSolt), Integer.valueOf(leaveEndSolt), leave_type, leave_days, userid);
					if(validVaction.containsKey(ResultMessage.Fail)){
						throw new ApiExcetpion(ReturnCode.PRAMS_IS_NULL, validVaction.get(ResultMessage.Fail));
					}
				}
				// Element data = datas.get(0);
				// String formData = data.getText();
				// if (StringUtil.isNotEmpty(formData)) {
				processCmd.setFormData(datas);
				// }
			}
			...
			// processCmd.setVariables(variables);
			// }
			if (StringUtils.isNotEmpty(request.getParameter("var"))) {
				Map<String, Object> vars = (Map<String, Object>) JSONObject.fromObject(request.getParameter("var"));
				if (vars.size() > 0) {
			...
			if (StringUtils.isNotEmpty(request.getParameter("runId"))) {
				...
				if (StringUtil.isNotEmpty(runIdStr)) {
					...
					if (BeanUtils.isEmpty(processRun)) {
						...
						throw new ApiExcetpion(ReturnCode.NO_DATA_FOUND, "草稿已被删除");
					}
			...
			if (taskId != -1) {
				...
				if (task != null) {
					if (StringUtil.isEmpty(task.getFlowrunid())) {
						task.setFlowrunid(processRun.getRunId() + "");
					} else {
						task.setFlowrunid(task.getFlowrunid() + "," + processRun.getRunId());
					}
