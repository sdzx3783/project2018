...
public class TaskCompleteListener extends BaseTaskListener {
	...
	protected void execute(DelegateTask delegateTask, String actDefId, String nodeId) {
		...
		//各类流程特殊处理
		updBpmCommon(delegateTask,processCmd);
		//更新执行堆栈里的执行人员及完成时间等
	...
	private void updBpmCommon(DelegateTask delegateTask,ProcessCmd processCmd){
		try{
			bpmCommonService.handleBpmCommon( delegateTask, processCmd);
		}catch(Exception e){
