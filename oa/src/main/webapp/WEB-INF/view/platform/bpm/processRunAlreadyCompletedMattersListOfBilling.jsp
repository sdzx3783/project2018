...
<body>
    ...
    <div class="oa-pdb20 oa-mgh20">
        <div id="oa_list_content" class="oa-table-scroll-wrap">
        	<c:set var="startNum" value="${(pageBeanprocessRunItem.currentPage-1)*pageBeanprocessRunItem.pageSize+1}"></c:set>
            <display:table name="processRunList" id="processRunItem" requestURI="alreadyCompletedMattersList.ht" sort="external" class="oa-table--default oa-table--nowrap">
                ...
                <display:column titleKey="申请人" sortable="true" sortName="creator">
                    ${processRunItem.processTask.creator}
                </display:column>
                <display:column property="processTask.subject" titleKey="流程名称" sortable="true" sortName="subject"></display:column>
                <display:column title="购买方名称" >${processRunItem.contractBillingApplication.buyerName}</display:column>         
                <display:column title="货物或应劳务、服务名称" >${processRunItem.contractBillingApplication.goodsName}</display:column>       
                <display:column title="开票金额" >${processRunItem.contractBillingApplication.billing_money}</display:column>         
                <display:column title="备注" >${processRunItem.contractBillingApplication.remark}</display:column>  
                <display:column titleKey="状态" sortable="true" sortName="status">
                    <f:processStatus status="${processRunItem.processTask.status}"></f:processStatus>
                </display:column>
                <display:column  titleKey="创建时间" sortable="true" sortName="createtime">
                    <fmt:formatDate value="${processRunItem.processTask.createtime}" pattern="yyyy-MM-dd HH:mm:ss"/>
                </display:column>
                <display:column titleKey="持续时间">                    
                    <c:choose>
                        <c:when test="${processRunItem.processTask.status==10 or processRunItem.processTask.status ==2 or processRunItem.processTask.status ==3}">
                            ${f:getTime(processRunItem.processTask.duration)}
                        </c:when>
                        <c:otherwise>
                            ${f:getDurationTime(processRunItem.processTask.createtime)} 
                        </c:otherwise>
                    </c:choose>
                </display:column>
                <c:if test="${hasGlobalFlowNo }">
                    <%-- <display:column property="globalFlowNo" class="notscroll" title="工单号" sortable="true"  sortName="globalFlowNo"></display:column> --%>
	                 <display:column  titleKey="工单号" sortable="true" sortName="globalFlowNo">
	                    <a name="globalFlowNo" onclick="showDetail(this)" href="#${processRunItem.processTask.runId}"  action="${ctx}/platform/bpm/processRun/info.ht?runId=${processRunItem.processTask.runId}&prePage=myFinishedTask" title='${processRunItem.processTask.globalFlowNo}' >${processRunItem.processTask.globalFlowNo}</a>
	                </display:column>
                </c:if>
              <%--   <display:column  titleKey="请求标题" sortable="true" sortName="subject">
                    <a name="processDetail" onclick="showDetail(this)" href="#${processRunItem.processTask.runId}"  action="info.ht?runId=${processRunItem.processTask.runId}&prePage=myFinishedTask" title='${processRunItem.processTask.subject}' >${processRunItem.processTask.subject}</a>
                </display:column> --%>                                                                                 
            </display:table>
        </div>
