...
<div class="noprint">
		...
		<div class="toolBar">
			<c:choose>
				<c:when test="${ isManage==0 }">
						...
						<c:if test="${isCanBack}">
						<div class="group"><a id="btnReject" class="link reject" ><span></span>驳回到上一步</a></div>
						<div class="l-bar-separator"></div>
					...
					<!-- 	<div class="l-bar-separator"></div>
						...
						<c:if test="${bpmDefinition.defId=='10000000480116' &&  task.name == '11.解除劳动合同证明'}">
							<div class="group"><a class="link print" onclick="specialContractPrint();"><span></span>打印模板</a></div>
						</c:if>
						
						<c:if test="${bpmDefinition.subject=='收入证明盖章申请'}">
	...
	<%-- 					<c:if test="${bpmDefinition.defId=='10000001640041' &&  task.name == '办文人员办文'}">
							<div class="group"><a id="sendReceptDoc" class="link" ><span></span>传阅</a></div>
						</c:if>
						<c:if test="${bpmDefinition.defId=='10000003390004' &&  task.name == '发文'}">
							<div class="group"><a  href="#" class="link" ><span></span>归档</a></div>
						</c:if> --%>
						<c:if test="${isExtForm}">
							<c:choose>
