...
<body>
            ...
            <div class="oa-accordion__content active">
                <form class="oa-clear" method="post" id="matterform" action="pendingMattersList.ht?porIndex=${porIndex}&tabIndex=${tabIndex}">
                  ...
                  <%--   <div class="oa-fl oa-mgb10">
					...
					<input type="hidden" name="nodePath" value="${param['nodePath'] }">
    ...
    <div class="oa-pdb20 oa-mgh20"> 
        <div id="oa_list_content" class="oa-table-scroll-wrap">
            ...
            <display:table name="infoMap" id="taskItem" requestURI="pendingMattersList.ht" sort="external" class="oa-table--default oa-table--nowrap">
                <display:column title="${checkAll}" media="html">
                        <c:choose>
                        <c:when test="${taskItem.processTask.description eq '15' || taskItem.processTask.description eq '38'}">
                            <input type="checkbox" class="pk" name="id"  disabled="disabled" value="${taskItem.processTask.id}">
                        </c:when>
                        <c:otherwise >
                            <input type="checkbox" class="pk" name="id"  value="${taskItem.processTask.id}">
                        </c:otherwise>
                    </c:choose>
                </display:column>
                <display:column  title="申请人" sortable="true" sortName="creator">
                    ${taskItem.processTask.creator}
                </display:column>
                <display:column title="流程名称" >
                    <c:choose> 
                            <c:when test="${taskItem.processTask.description eq '36'}">
                                <a name="subject"  href="#${taskItem.processTask.id}" onclick="javascript:restdartTask(${taskItem.processTask.id})" style="color:${warningSet[taskItem.processTask.priority].color}" title='${taskItem.processTask.subject}'  <c:if test="${taskItem.processTask.hasRead == 0}"> class='message close-message'</c:if><c:if test="${taskItem.processTask.hasRead != 0}"> class='message open-message'</c:if> >${f:subString(taskItem.processTask.subject)}</a>
                            </c:when>
                        <c:otherwise>
                                <a name="subject"  href="#${taskItem.processTask.id}" onclick="javascript:executeTask(${taskItem.processTask.id})" style="color:${warningSet[taskItem.processTask.priority].color}" title='${taskItem.processTask.subject}' <c:if test="${taskItem.processTask.hasRead == 0}"> class='message close-message'</c:if><c:if test="${taskItem.processTask.hasRead != 0}"> class='message open-message'</c:if> >${f:subString(taskItem.processTask.subject)}</a>
                        </c:otherwise>
                    </c:choose>
                </display:column>
                <display:column  title="申请部门">
                    ${taskItem.processTask.orgName}
                </display:column>
                <display:column title="任务名称" property="processTask.name"></display:column>        
                <display:column title="购买方名称" >${taskItem.contractBillingApplication.buyerName}</display:column>         
                <display:column title="货物或应劳务、服务名称" >${taskItem.contractBillingApplication.goodsName}</display:column>       
                <display:column title="开票金额" >${taskItem.contractBillingApplication.billing_money}</display:column>         
                <display:column title="备注" >${taskItem.contractBillingApplication.remark}</display:column>                                                                 
                <display:column title="状态">
                    <c:choose>
                        <c:when test="${taskItem.processTask.taskStatus==1}">审批中</c:when>
                        <c:when test="${taskItem.processTask.taskStatus==5}">已撤销</c:when>
                        <c:when test="${taskItem.processTask.taskStatus==6}">已驳回</c:when>
                        <c:when test="${taskItem.processTask.taskStatus==7}">已追回</c:when>
                    </c:choose>
                </display:column>
                <display:column title="待办类型">
                    <c:choose>
                        <c:when test="${taskItem.processTask.description=='-1'}">
                            <span class="green">待办</span>
                        </c:when>
                        <c:when test="${taskItem.processTask.description eq '21' }" >
                            <span class="brown">转办</span>
                        </c:when>
                        <c:when test="${taskItem.processTask.description eq '15' }" >
                            <span class="orange">沟通意见</span>
                        </c:when>
                        <c:when test="${taskItem.processTask.description eq '26' }" >
                            <span class="brown">代理</span>
                        </c:when>
                        <c:when test="${taskItem.processTask.description eq '38' }" >
                            <span class="red">流转意见</span>
                        </c:when>
                    </c:choose>
                </display:column>            	
                <c:if test="${hasGlobalFlowNo }">
                    <display:column property="processTask.globalFlowNo" title="工单号" class="notscroll" ></display:column>
                </c:if> 
            </display:table>
        </div>
