	...
	<div class="panel">
		<div class="panel-top">
			<div class="tbar-title" style="display: block;">
				<span class="tbar-label">假期类型管理列表</span>
			</div>
		...
		<div class="panel-body">
		    ...
		    <display:table name="atsHolidayTypeList" id="atsHolidayTypeItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
				<display:column title="${checkAll}" media="html" style="width:30px;">
			  		<input type="checkbox" class="pk" name="id" value="${atsHolidayTypeItem.id}">
				</display:column>
				...
				<display:column title="是否系统预置" sortable="true" sortName="IS_SYS">
						<c:choose>
							<c:when test="${atsHolidayTypeItem.isSys==1}">
				...
				<display:column title="状态" sortable="true" sortName="STATUS">
						<c:choose>
							<c:when test="${atsHolidayTypeItem.status==1}">
				...
				<display:column title="异常标记">
					<c:choose>
						<c:when test="${atsHolidayTypeItem.abnormity==0}">
				...
				<display:column title="管理" media="html" class="rowOps">
					<c:if test="${atsHolidayTypeItem.isSys!=1}">
						<a href="del.ht?id=${atsHolidayTypeItem.id}" class="link del">删除</a>
						<a href="edit.ht?id=${atsHolidayTypeItem.id}" class="link edit">编辑</a>
					</c:if>
					<a href="get.ht?id=${atsHolidayTypeItem.id}" class="link detail">明细</a>
					<c:choose>
						<c:when test="${atsHolidayTypeItem.status==1}">
							<a href="enable.ht?id=${atsHolidayTypeItem.id}&enable=false" class="link lock" style="color:red;font-weight: bold;">禁用</a>
						</c:when>
						<c:otherwise>
							<a href="enable.ht?id=${atsHolidayTypeItem.id}&enable=true" class="link lock" style="color:green;font-weight: bold;">启用</a>
						</c:otherwise>
					</c:choose>
				</display:column>
			</display:table>
			<hotent:paging tableId="atsHolidayTypeItem"/>
