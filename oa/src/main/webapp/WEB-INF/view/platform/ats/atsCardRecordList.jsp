 ...
 -->				</div>	
			...
			<div class="panel-search">
				<form id="searchForm" method="post" action="list.ht">
					<div class="row">
						<span class="label">考勤卡号:</span><input type="text" name="Q_cardNumber_SL"  class="inputText" value="${param['Q_cardNumber_SL']}" />
						<span class="label">打卡日期 从:</span> <input  name="Q_begincardDate_DL"  class="inputText date" value="${param['Q_begincardDate_DL']}"/>
						<span class="label">至: </span><input  name="Q_endcardDate_DG" class="inputText date"   value="${param['Q_endcardDate_DG']}"/>
						<span class="label">打卡来源:</span>
							<select name="Q_cardSource_SN" value="${param['Q_cardSource_SN']}">
								<option value="">请选择</option>
								<option value="1" <c:if test="${param['Q_cardSource_SN'] == '1'}">selected</c:if>>补打卡</option>	
								<option value="2" <c:if test="${param['Q_cardSource_SN'] == '2'}">selected</c:if>>Excel</option>
								<option value="3" <c:if test="${param['Q_cardSource_SN'] == '3'}">selected</c:if>>Access</option>
								<option value="4" <c:if test="${param['Q_cardSource_SN'] == '4'}">selected</c:if>>TEXT</option>
							
							</select>
						
						<span class="label">打卡位置:</span><input type="text" name="Q_cardPlace_SL"  class="inputText" value="${param['Q_cardPlace_SL']}"  />
					</div>
				</form>
			</div>
		</div>
