	...
	<div class="panel">
			...
			<div class="panel-search">
				<form id="searchForm" method="post" action="list.ht?__FILTERKEY__=${busQueryRule.filterKey}&__IS_QUERY__=0">
					<div class="row">
						...
						<span class="label">人员组织:</span><input type="text" name="Q_creator_SL"  class="inputText"  value="${param['Q_creator_SL']}"/>
						<span class="label">创建时间 从:</span> <input  name="Q_begincreatetime_DL"  class="inputText date" value="${param['Q_begincreatetime_DL']}" />
						<span class="label">至: </span><input  name="Q_endcreatetime_DL" class="inputText date" value="${param['Q_endcreatetime_DG']}" />
					</div>
				</form>
			</div>
		</div>
		<div class="panel-body">
		    ...
		    <display:table name="sysKnowPerRefList" id="sysKnowPerRefItem" requestURI="list.ht?__FILTERKEY__=${busQueryRule.filterKey}&__FILTER_FLAG__=${busQueryRule.filterFlag}" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
				...
				<display:column title="管理"  media="html" style="width:130px">
					...
					<a href="get.ht?id=${sysKnowPerRefItem.id}&id=${id}" class="link detail">明细</a>
				</display:column>
			</display:table>
