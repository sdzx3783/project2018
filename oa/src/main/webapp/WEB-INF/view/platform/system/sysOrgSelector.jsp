			...
			<display:column title="${checkAll}" media="html" style="width:30px;">
				<c:choose>
					<c:when test="${isSingle==false}">
						<input type="checkbox" class="pk" name="orgId" value="${sysOrgItem.orgId}#${fn:replace(sysOrgItem.orgPathname,'/深水咨询/','')}">
					</c:when>
					<c:otherwise>
						<input type="radio" class="pk" name="orgId" value="${sysOrgItem.orgId}#${fn:replace(sysOrgItem.orgPathname,'/深水咨询/','')}">
					</c:otherwise>
				</c:choose>
