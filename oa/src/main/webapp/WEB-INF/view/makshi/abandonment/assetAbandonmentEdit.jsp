...
<head>
	...
	<script type="text/javascript">
		$(function() {
			...
			var frm=$('#assetAbandonmentForm').form();
			...
			$("a.run").click(function() {
				frm.ajaxForm(options);
				$("#saveData").val(0);
				$('#assetAbandonmentForm').attr("action","run.ht");
				if(frm.valid()){
					if(WebSignPlugin.hasWebSignField){
						WebSignPlugin.submit();
					}
			...
			if (obj.isSuccess()) {
				$.ligerDialog.confirm(obj.getMessage()+",是否继续操作","提示信息", function(rtn) {
					if(rtn){
						...
						window.location.href = "${ctx}/makshi/abandonment/assetAbandonment/list.ht";
					}
				});
			} else {
...
<div class="panel" style="height:100%;overflow:auto;">
	<div class="panel-top">
		<div class="tbar-title">
		    <c:choose>
			    <c:when test="${not empty assetAbandonmentItem.id}">
			        <span class="tbar-label"><span></span>编辑资产报废表</span>
			    </c:when>
			    <c:otherwise>
			        <span class="tbar-label"><span></span>添加资产报废表</span>
			    </c:otherwise>	
		    </c:choose>
		</div>
	...
	<form id="assetAbandonmentForm" method="post" action="save.ht">
		<div type="custform">
 ...
 <tbody>
  ...
  <tr>
   ...
   <input type="text" name="m:asset_abandonment:standard" lablename="质量要求及验收标准" class="inputText" value="${assetAbandonment.standard}" validate="{maxlength:50}" isflag="tableflag" /> </span> </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td style="width: 35%" class="formInput"> 
   <span name="editable-input" style="display:inline-block;" isflag="tableflag">
    <input type="text" name="m:asset_abandonment:specification" lablename="规格型号" class="inputText" value="${assetAbandonment.specification}" validate="{maxlength:50}" isflag="tableflag" /> </span> </td>
  ...
  <tr style="display:none">
   ...
   <td class="formInput" colspan="3" rowspan="1"><br /> <span name="editable-input" style="display:inline-block;padding:2px;" class="selectinput"> <select name="m:asset_abandonment:state" lablename="审批状态" controltype="select" validate="{empty:false}" val="${assetAbandonment.state}"> <option value="0">审核中</option> <option value="1">办理中</option> <option value="2">驳回</option> <option value="3">结束</option> </select> </span> </td>
		...
		<input type="hidden" name="id" value="${assetAbandonment.id}"/>
