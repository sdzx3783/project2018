		...
		<div id="oa_list_content" class="oa-table-scroll-wrap">
			<table class="oa-table--default oa-table--nowrap">
				...
				<tr>
					<td>${a.index+1}</td>
					<td>
						<%-- <a href="/makshi/plan/plan/detail.ht?id=${planItem.id}&type=1" > --%>
						${username}-
					...
					<td><fmt:formatDate value='${planItem.ctime }' pattern='yyyy-MM-dd HH:mm:ss'/></td>
					<td>
					...
					<td>
						<c:if test="${planItem.ispub==1}">是</c:if>
						<c:if test="${planItem.ispub==0}">否</c:if>
					</td> 
					<td>
						<a href="/makshi/plan/plan/edit.ht?id=${planItem.id}" class="oa-button-label"><span></span>编辑</a>
						<c:if test="${planItem.ispub==0}">
						<a onclick="delPlan(${planItem.id})" href="#" class="oa-button-label"><span></span>删除</a>
						</c:if>
					</td>
				</tr>
				</c:forEach>
