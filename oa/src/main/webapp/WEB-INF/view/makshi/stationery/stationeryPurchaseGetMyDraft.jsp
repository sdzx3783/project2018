			...
			$.post("run.ht?isList=1&id="+id+"&runId="+runId,function(responseText){
				...
				if (obj.isSuccess()) {
					$.ligerDialog.success("启动流程成功！", "成功", function(rtn) {
						this.close();
						window.location.href = "${ctx}/makshi/stationery/stationeryPurchase/getMyDraft.ht";
					});
				} else {
		...
		<div class="panel-body">
		    ...
		    <display:table name="stationeryPurchaseList" id="stationeryPurchaseItem" requestURI="getMyDraftJson.ht" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
				...
				<display:column title="管理" media="html" style="width:220px">
					<c:if test="${stationeryPurchaseItem.runId==0}">
						<a href="del.ht?id=${stationeryPurchaseItem.id}" class="link del"><span></span>删除</a>
						<a href="javascript:;" onclick="startFlow(this,'${stationeryPurchaseItem.id}','${stationeryPurchaseItem.runId}')" class="link run"><span></span>提交</a>
					</c:if>
				</display:column>
			</display:table>
