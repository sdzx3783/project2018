	...
	<div class="oa-main-wrap">
		<form id="stagetaskForm" method="post" action="stageTaskSave.ht">
			<table class="oa-table">
				<tr>
					<th>任务名称：</th>
					<td>${projectStageTask.taskname }</td>
				</tr>
				<tr>
					<th>模板文件：</th>
					<td>
						<div name="div_attachment_container">
							...
							<textarea style="display: none" controltype="attachment" name="file" lablename="附件" validate="{}">${projectStageTask.templatefile}</textarea>
						</div>
					</td>
				</tr>
				<tr>
					<th>备注：</th>
					<td>
						${projectStageTask.remark}
					</td>
				</tr>
				<tr>
					<th>任务类型：</th>
					<td>
						<c:if test="${projectStageTask.tasktype==1 }">表单填写类</c:if>
						<c:if test="${projectStageTask.tasktype==2 }">资料上传类</c:if>
						<c:if test="${projectStageTask.tasktype==3 }">流程审批类</c:if>
					</td>
				</tr>
				...
				<tr class="gnjdtype" <c:if test="${projectStageTask.tasktype!=3 }">style="display:none" </c:if>>
					<td></td>
					<td>
						<input type="checkbox" id="isexamine" disabled="disabled" name="isexamine" value="1" <c:if test="${projectStageTask.isexamine}">checked="checked"</c:if>>
						是否需要审批 <span id="relationflowid" <c:if test="${!projectStageTask.isexamine}">style="display:none"</c:if>>
						关联流程：${projectStageTask.flowid}
					</td>
				</tr>
				<tr class="zlsctype">
					<td></td>
					<td>
						<input type="checkbox" id="ismore" disabled="disabled" name="ismore" value="1" <c:if test="${projectStageTask.ismore}">checked="checked"</c:if>>
						是否需要提交多次
					</td>
				</tr>
