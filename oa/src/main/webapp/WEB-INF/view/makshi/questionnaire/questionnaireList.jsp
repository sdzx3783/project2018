...
<body class="oa-mw-page">
	...
	<c:if test="${sessionScope.LOGIN_USER_ID == 1}">
    	<div id="oa_list_operation" class="oa-pdb20 oa-mgh20 oa-mgl20">
		    <a class="oa-button oa-button--primary oa-button--blue" href="edit.ht">新建调查</a>
		</div>
	</c:if>
				...
				<c:choose>
					...
					<c:if test="${sessionScope.LOGIN_USER_ID == questionnaireItem.creater_id
					 && questionnaireItem.creater_partin!=null && questionnaireItem.creater_partin==true}">
						<a href="ballot.ht?id=${questionnaireItem.id}" class="oa-button-label edit"><span></span>投票</a>
					</c:if>
					<c:if test="${sessionScope.LOGIN_USER_ID != questionnaireItem.creater_id}">
						<a href="ballot.ht?id=${questionnaireItem.id}" class="oa-button-label edit"><span></span>投票</a>
					</c:if>
					<c:if test="${sessionScope.LOGIN_USER_ID == questionnaireItem.creater_id
					 && questionnaireItem.status==0}">
						<a href="del.ht?id=${questionnaireItem.id}" class="oa-button-label del"><span></span>删除</a>
						<a href="edit.ht?id=${questionnaireItem.id}" class="oa-button-label edit"><span></span>编辑</a>
					</c:if>
					<c:if test="${sessionScope.LOGIN_USER_ID == questionnaireItem.creater_id}">
						<a href="get.ht?id=${questionnaireItem.id}" class="oa-button-label edit"><span></span>详情</a>
					</c:if>
				</display:column>
			</display:table>
	    </div>
