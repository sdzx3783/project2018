...
<head>
	...
	<script type="text/javascript">
		$(function() {
			...
			var frm=$('#practiceregistForm').form();
			...
			if (obj.isSuccess()) {
				$.ligerDialog.confirm(obj.getMessage()+",是否继续操作","提示信息", function(rtn) {
					if(rtn){
						...
						window.location.href = "${ctx}/makshi/title/practiceregist/list.ht";
					}
				});
			} else {
...
<div class="panel" style="height:100%;overflow:auto;">
	<div class="panel-top">
		<div class="tbar-title">
		    <c:choose>
			    <c:when test="${not empty practiceregistItem.id}">
			        <span class="tbar-label"><span></span>编辑个人执业资格初始注册及转入</span>
			    </c:when>
			    <c:otherwise>
			        <span class="tbar-label"><span></span>添加个人执业资格初始注册及转入</span>
			    </c:otherwise>	
		    </c:choose>
		</div>
	...
	<form id="practiceregistForm" method="post" action="save.ht">
		<div type="custform">
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td style="width: 35%; word-break: break-all;" class="formInput" rowspan="1" colspan="3"><br /> <span name="editable-input" style="display:inline-block;" isflag="tableflag"> <textarea name="m:PracticeRegist:remark" lablename="备注" class="l-textarea" rows="5" cols="40" validate="{maxlength:1000}" isflag="tableflag">${practiceregist.remark}</textarea> </span> </td> 
		...
		<input type="hidden" name="id" value="${practiceregist.id}"/>
