			...
			<div class="file-list">
				<c:forEach items="${docList}" var="doc" >
					<div class="file-item dir-item">
						<a href="docList.ht?docId=${doc.docid}">${doc.docname}</a>
						<%-- <a href="getListByYear.ht?docId=${doc.docid}&&year=${doc.docname}">${doc.docname}</a> --%>
					</div>
				</c:forEach>
			</div>
			 <c:if test='${canCreate}'>
...
<script>
	$(function(){
		...
		$(".del_newcat").click(function(){
			$.post("/makshi/dispatch/dispatch/delDocList.ht",{},function(data){
				//closeAll();
				var d = dialog({
					title : "删除分类",
					fixed : true,
