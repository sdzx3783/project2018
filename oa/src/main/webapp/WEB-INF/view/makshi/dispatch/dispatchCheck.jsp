		...
		<div class="panel-body">
		    ...
		    <display:table name="dispatchList" id="dispatchItem" requestURI="check.ht" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
				<display:column title="${checkAll}" media="html" style="width:30px;">
			  		<input type="checkbox" class="pk" name="id" value="${dispatch.id}">
				</display:column>
				...
				<display:column title="状态">
					<c:if test="${dispatchItem.checkState==1}">已核稿</c:if>
					<c:if test="${dispatchItem.checkState!=1}">待核稿</c:if>
				</display:column>
				
				
				<display:column title="操作" media="html" style="width:120px">
					<c:if test="${dispatchItem.runId==0}">
						<a href="del.ht?id=${dispatch.id}" class="link del"><span></span>删除</a>
					</c:if>
					<c:if test="${dispatchItem.checkState!=1}">
					 <a href="javascript:;"  onclick="executeTask(${dispatchItem.taskId})">核稿</a> 
					 </c:if>
				<%-- 	<a href="edit.ht?id=${dispatch.id}" class="link edit"><span></span>编辑</a>
					<a href="get.ht?id=${dispatch.id}" class="link detail"><span></span>明细</a> --%>
				</display:column>
			</display:table>
