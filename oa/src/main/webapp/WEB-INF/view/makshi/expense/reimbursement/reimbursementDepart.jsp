...
<body>
	<div id="oa_list_title" class="oa-mgb20 oa-project-title">
	    	<h2 class='executivetitle'>部门人员费用汇总表</h2>
	</div>
	<div  >
        <div class="col-md-12">
			<form class="form-inline" role="form" method="post" action="/makshi/expense/reimbursement/depart.ht" id="search-form">
		         <div class="form-body">
		        	 <div class="form-group">
		               	<label class="control-label">部<label>&nbsp;</label>门</label>
		               	<select id="orgId" name='orgId' class='form-control'  style="width: 250px !important;">
		            ...
		            <a  href="javascript:;" class="oa-button oa-button--primary oa-button--blue ${empty types?'hidden':'' } " id="btnExport">导出</a>
		         </div>
		     </form>
		 </div>
    </div>
					...
					<c:choose>
						...
						<c:otherwise>
							...
							<tr>
								...
								<c:forEach items="${users }" var="d" varStatus="i">
									<th class="center" rowspan="2">${d.userName }<c:if test="${not empty d.userNum }">/${d.userNum }</c:if></th>
								</c:forEach>
							...
							<c:forEach items="${types }" var="t">
								<tr>
									...
									<c:forEach items="${users }" var="d">
										<c:set var="v">${d.userName}${t.typeName}</c:set>
							...
							<tr>
								...
								<c:forEach items="${users }" var="d">
									<th class="center"><fmt:formatNumber value="${empty dateTotalValuesmaps[d.userName]?0: dateTotalValuesmaps[d.userName]}" pattern="##.##"></fmt:formatNumber> </th>
								</c:forEach>
		...
		$(function(){
			$("#btnExport").click(function(){
		        $('#tb_common_show').tableExport({
			        ...
			        escape:'false',
			        htmlContent :true,
			        fileName: $(".executivetitle").text()+('('+$("#orgId option:selected").text()+$("#entrydateStart").val()+'—'+$("#entrydateEnd").val()+')')
		        });
		    });
		})
	</script>
