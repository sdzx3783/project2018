...
<body>
    <div id="oa_list_title" class="oa-mgb10 oa-project-title">
        <h2  class='executivetitle'>${syear}<c:if test="${eyear!=null}">-${eyear}</c:if>年深水咨询主要客户</h2>
    </div>
	...
	<div class="col-md-12" style="padding-bottom: 20px">
		<div class="oa-fl oa-mgb10 oa-mgh20">
			<select id="tabStatics" name="tabStatics" class="form-control" style="display: inline-block;" >
				...
				<option value="2" selected="selected"  data-url="/makshi/contract/contractinfo/customers.ht">深水咨询近年最大客户</option>
    ...
    <div class="oa-pdb20 oa-mgh20">
        <div id="oa_list_content" class="oa-table-scroll-wrap">
			<c:set var="startNum" value="${(pageBeangoodscustomersItem.currentPage)*pageBeancustomersItem.pageSize}"></c:set>
		    <display:table export="true" name="customerslist" id="customersItem" requestURI="#" sort="external" cellpadding="1" cellspacing="1"  class="oa-table--default oa-table--nowrap">
		   	<c:set var="startNum" value="${startNum+1}"></c:set>
				<display:column title="序号">${startNum}</display:column>
				<display:column title="业主单位名称" class="w30"><span>${customersItem.department}</span></display:column>
				<display:column title="总金额/万元" class="w30">${customersItem.allAccount}</display:column>
				<display:column title="占比(%)" class="w30">${customersItem.rate}</display:column>
			</display:table>
		</div>			
		<hotent:paging tableId="customersItem"/>
	</div>
	<script type="text/javascript">
	 $(function(){
