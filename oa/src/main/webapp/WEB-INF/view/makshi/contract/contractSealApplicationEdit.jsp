...
<head>
	...
	<script type="text/javascript">
		$(function() {
			...
			var frm=$('#contractSealApplicationForm').form();
				...
				if(frm.valid()){
					...
					if(OfficePlugin.officeObjs.length>0){
						OfficePlugin.submit(function(){
							frm.handleFieldName();
							$('#contractSealApplicationForm').submit();
						});
					}else{
						frm.handleFieldName();
						$('#contractSealApplicationForm').submit();
			...
			$("a.run").click(function() {
				frm.ajaxForm(options);
				$("#saveData").val(0);
				$('#contractSealApplicationForm').attr("action","run.ht");
				if(frm.valid()){
					if(WebSignPlugin.hasWebSignField){
						WebSignPlugin.submit();
					}
			...
			if (obj.isSuccess()) {
				$.ligerDialog.confirm(obj.getMessage()+",是否继续操作","提示信息", function(rtn) {
					if(rtn){
						...
						window.location.href = "${ctx}/makshi/contract/contractSealApplication/list.ht";
					}
				});
			} else {
...
<div class="panel" style="height:100%;overflow:auto;">
	<div class="panel-top">
		<div class="tbar-title">
		    <c:choose>
			    <c:when test="${not empty contractSealApplicationItem.id}">
			        <span class="tbar-label"><span></span>编辑合同盖章申请</span>
			    </c:when>
			    <c:otherwise>
			        <span class="tbar-label"><span></span>添加合同盖章申请</span>
			    </c:otherwise>	
		    </c:choose>
		</div>
	...
	<form id="contractSealApplicationForm" method="post" action="save.ht">
		<div type="custform">
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td style="width: 35%; word-break: break-all;" class="formInput" rowspan="1" colspan="3"><br /> <span name="editable-input" style="display:inline-block;" isflag="tableflag"> <textarea name="m:contract_seal_application:remark" lablename="备注" class="l-textarea" rows="5" cols="40" validate="{maxlength:1000}" isflag="tableflag">${contractSealApplication.remark}</textarea> </span> </td> 
  </tr> 
  <tr> 
   ...
   <td style="width: 35%; word-break: break-all;" class="formInput" rowspan="1" colspan="3"><br /></td> 
		...
		<input type="hidden" name="id" value="${contractSealApplication.id}"/>
