	...
	<form id="docForm" action="list.ht" method="post">
		...
		<div class="panel-toolbar">
			<div class="toolBar">
				...
				<c:if test='${supId==0}'>
					<select name="orgId" id="orgs" onchange="orgChange()">
						<c:forEach items="${orgs}" var="org" >
							<option value="${org.orgId }" <c:if test='${orgId==org.orgId}'>selected="selected"</c:if>>${org.orgName}</option>
						</c:forEach>
					</select>
				</c:if>
				...
				<div class="btn-wrap">
					<c:if test='${canCreate}'>
						<a class="btn create_newcat" href="javascript:;" data-url="edit.ht?orgId=${orgId }&supId=${supId}"><span class="icon icon-folder-add"></span><span class="text">新建分类</span></a>
					</c:if>
					<c:if test='${supId!=0}'>
						<a class="btn " href="list.ht?orgId=${orgId }&supId=${lastSupId}"><span class="icon icon-arrow-back"></span><span class="text">返回</span></a>
					</c:if>
				</div>
			</div>	
		</div>
		...
		<div class="content-wrap">
			<div class="file-list">
				<c:forEach items="${docList}" var="doc" >
					<c:if test='${doc.doctype==0}'>
						<div class="file-item dir-item">
							<c:choose>
								<c:when test="${doc.isRead }">
									<a href="list.ht?orgId=${doc.orgid }&supId=${doc.docid}">${doc.docname}</a>
								</c:when>
								<c:otherwise>
									<a href="javascript:;" onclick="alert('无访问权限');">${doc.docname}</a>
								</c:otherwise>
							</c:choose>
						</div>
					</c:if>
					<c:if test='${doc.doctype==1}'>
						<div class="file-item">
							<c:choose>
								<c:when test="${doc.isRead }">
									<a href="filelist.ht?docId=${doc.docid }">${doc.docname}</a>
								</c:when>
								<c:otherwise>
									<a href="javascript:;" onclick="alert('无访问权限');">${doc.docname}</a>
								</c:otherwise>
			...
			<c:if test='${canCreate}'>
				<div class="file-item add-item">
					<a href="javascript:;" class="create_newcat" data-url="edit.ht?orgId=${orgId }&supId=${supId}">新建分类</a>
				</div>
			</c:if>
		</div>
	</form>
...
<script>
	$(function(){
		...
		$(".create_newcat").click(function(){
			$.post("/makshi/doc/docinfo/" + $(this).data("url"),{},function(data){
				closeAll();
				var d = dialog({
					title : "新建分类",
					fixed : true,
