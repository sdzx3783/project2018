			...
			<c:otherwise>
				<c:forEach items="${list }" var="e" varStatus="v">
					<tr>
						...
						<td><a href="/makshi/assess/employee/detail.ht?id=${e.id }">${e.employee }</a></td>
						<td>
							<a href="/makshi/assess/employee/detail.ht?id=${e.id }"><fmt:formatDate value="${e.evaluation_at }" pattern="yyyy年MM月"/></a>
						</td>
						<td>${e.status==1?'未提交':'已提交' }</td>
						<td>
							<fmt:formatNumber value="${e.status==3?e.total:'' }" pattern="##.##"></fmt:formatNumber>
						</td>
						<td>
							<c:if test="${e.status==1 }">
								<a href="/makshi/assess/employee/edit.ht?id=${e.id }" class="oa-button-label">编辑</a>
