...
<body class="oa-mw-page">
	...
	<div class="oa-pdb20 oa-mgh20">
		<div id="oa_list_content" class="oa-table-scroll-wrap">
		    <display:table name="hjOperationPlanList" id="hjOperationPlanItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" class="oa-table--default oa-table--nowrap">
				<c:set var="startNum" value="${startNum+1}"></c:set>	
				<display:column title="序号">
			  		${startNum}
				</display:column>
				<display:column title="方案名称">
			  		${hjOperationPlanItem.n_name}
				</display:column>
				<display:column title="方案内容">
			  		${hjOperationPlanItem.content}
				</display:column>
				<display:column title="项目名称">
			  		${hjOperationPlanItem.project_name}
				</display:column>
				<display:column title="上传人">
			  		${hjOperationPlanItem.name}
				</display:column>
				<display:column title="所属部门">
			  		${hjOperationPlanItem.department}
				</display:column>
				<display:column title="上传日期">
			  		<fmt:formatDate value='${hjOperationPlanItem.date}' pattern='yyyy-MM-dd'/>
				</display:column>
				<display:column title="备注">
			  		${hjOperationPlanItem.note}
				</display:column>
				<display:column title="管理" media="html">
					<a href="get.ht?id=${hjOperationPlanItem.id}" class="oa-button-label">明细</a>
				</display:column>
			</display:table>		
		</div>

		<hotent:paging tableId="hjOperationPlanItem"/>
	</div>
</body>
</html>
