...
<head>
	...
	<script type="text/javascript">
		$(function() {
			...
			var frm=$('#hjTechnicalReportExaminForm').form();
				...
				if(frm.valid()){
					...
					if(OfficePlugin.officeObjs.length>0){
						OfficePlugin.submit(function(){
							frm.handleFieldName();
							$('#hjTechnicalReportExaminForm').submit();
						});
					}else{
						frm.handleFieldName();
						$('#hjTechnicalReportExaminForm').submit();
			...
			$("a.run").click(function() {
				frm.ajaxForm(options);
				$("#saveData").val(0);
				$('#hjTechnicalReportExaminForm').attr("action","run.ht");
				if(frm.valid()){
					if(WebSignPlugin.hasWebSignField){
						WebSignPlugin.submit();
					}
			...
			if (obj.isSuccess()) {
				$.ligerDialog.confirm(obj.getMessage()+",是否继续操作","提示信息", function(rtn) {
					if(rtn){
						...
						window.location.href = "${ctx}/makshi/hj/hjTechnicalReportExamin/list.ht";
					}
				});
			} else {
...
<div class="panel" style="height:100%;overflow:auto;">
	<div class="panel-top">
		<div class="tbar-title">
		    <c:choose>
			    <c:when test="${not empty hjTechnicalReportExaminItem.id}">
			        <span class="tbar-label"><span></span>编辑技术报告报审</span>
			    </c:when>
			    <c:otherwise>
			        <span class="tbar-label"><span></span>添加技术报告报审</span>
			    </c:otherwise>	
		    </c:choose>
		</div>
	...
	<form id="hjTechnicalReportExaminForm" method="post" action="save.ht">
		<div type="custform">
 ...
 <tbody>
  ...
  <tr>
   ...
   <td class="formInput" style="width: 35%; -ms-word-break: break-all;"> <span name="editable-input" style="display:inline-block;" isflag="tableflag"> <textarea name="m:hj_Technical_report_examin:reportContent" lablename="报告内容" class="l-textarea" rows="3" cols="40" validate="{maxlength:1000}" isflag="tableflag">${hjTechnicalReportExamin.reportContent}</textarea> </span> </td>
  </tr>
  <tr>
   <td align="right" class="formTitle" style="width: 15%; -ms-word-break: break-all;">备注:</td>
   <td class="formInput" style="width: 35%; -ms-word-break: break-all;" rowspan="1" colspan="3"> <span name="editable-input" style="display:inline-block;" isflag="tableflag"> <textarea name="m:hj_Technical_report_examin:tag" lablename="备注" class="l-textarea" rows="3" cols="40" validate="{maxlength:1000}" isflag="tableflag">${hjTechnicalReportExamin.tag}</textarea> </span> </td>
  </tr>
 <%--  <tr>
  ...
  <tr>
  	...
  	<c:if test="${hjTechnicalReportExamin.validation=='F'}">
		<td align="right" class="formTitle" style="width: 15%; -ms-word-break: break-all;">是否需要审定: 否 </td>
		<td id="ischeck" class="formInput" style="width: 35%; -ms-word-break: break-all;" rowspan="1" colspan="3"></td>
	</c:if>
	<c:if test="${hjTechnicalReportExamin.validation=='T'}">
		<td align="right" class="formTitle" style="width: 15%; -ms-word-break: break-all;">是否需要审定: 是</td>
  ...
  <tr>
   ...
   <input  ctltype="attachment" name="m:hj_Technical_report_examin:enclosure" lablename="附件" isdirectupload="0" right="w" 
  ...
  <tr style="display: none;">
   ...
   <td class="formInput" style="-ms-word-break: break-all;" rowspan="1" colspan="1"> <span name="editable-input" style="display:inline-block;" isflag="tableflag"> <input type="text" name="m:hj_Technical_report_examin:projectTaskId" lablename="项目任务id" class="inputText" value="${hjTechnicalReportExamin.projectTaskId}" validate="{maxlength:20}" isflag="tableflag" /> </span> </td>
		...
		<input type="hidden" name="id" value="${hjTechnicalReportExamin.id}"/>
