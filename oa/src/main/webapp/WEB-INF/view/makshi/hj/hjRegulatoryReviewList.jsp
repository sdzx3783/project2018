...
<body class="oa-mw-page">
	...
	<div class="oa-pdb20 oa-mgh20">
		<div id="oa_list_content" class="oa-table-scroll-wrap">
			<display:table name="hjRegulatoryReviewList" id="hjRegulatoryReviewItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" class="oa-table--default oa-table--nowrap">
				<c:set var="startNum" value="${startNum+1}"></c:set>	
				<display:column title="序号">
			  		${startNum}
				</display:column>
				<display:column title="周报名称">
			  		${hjRegulatoryReviewItem.n_name}
				</display:column>
				<display:column title="周报内容">
			  		${hjRegulatoryReviewItem.content}
				</display:column>
				<display:column title="项目名称">
			  		${hjRegulatoryReviewItem.project_name}
				</display:column>
				<display:column title="提交人">
			  		${hjRegulatoryReviewItem.name}
				</display:column>
				<display:column title="提交日期">
			  		<fmt:formatDate value='${hjRegulatoryReviewItem.date}' pattern='yyyy-MM-dd'/>
				</display:column>
				<display:column title="所属部门">
			  		${hjRegulatoryReviewItem.department}
				</display:column>
				<display:column title="备注">
			  		${hjRegulatoryReviewItem.note}
				</display:column>
				<display:column title="管理" media="html">
					<a href="get.ht?id=${hjRegulatoryReviewItem.id}" class="oa-button-label">明细</a>
				</display:column>
			</display:table>
		</div>

		<hotent:paging tableId="hjRegulatoryReviewItem"/>
	</div>
