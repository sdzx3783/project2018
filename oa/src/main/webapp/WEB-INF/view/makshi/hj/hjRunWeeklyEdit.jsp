	...
	<script type="text/javascript">
		$(function() {
			...
			$("a.run").click(function() {
				frm.ajaxForm(options);
				$("#saveData").val(0);
				$('#hjRunWeeklyForm').attr("action","run.ht");
				if(frm.valid()){
					if(WebSignPlugin.hasWebSignField){
						WebSignPlugin.submit();
					}
			...
			if (obj.isSuccess()) {
				$.ligerDialog.confirm(obj.getMessage()+",是否继续操作","提示信息", function(rtn) {
					if(rtn){
						...
						window.location.href = "${ctx}/makshi/hj/hjRunWeekly/list.ht";
					}
				});
			} else {
...
<div class="panel" style="height:100%;overflow:auto;">
	<div class="panel-top">
		<div class="tbar-title">
		    <c:choose>
			    <c:when test="${not empty hjRunWeeklyItem.id}">
			        <span class="tbar-label"><span></span>编辑22</span>
			    </c:when>
			    <c:otherwise>
			        <span class="tbar-label"><span></span>添加22</span>
			    </c:otherwise>	
		    </c:choose>
		</div>
	...
	<form id="hjRunWeeklyForm" method="post" action="save.ht">
		<div type="custform">
 ...
 <tbody> 
  ...
  <tr style="display:none"> 
   ...
   <td class="formInput" colspan="1" rowspan="1"><br /></td> 
		...
		<input type="hidden" name="id" value="${hjRunWeekly.id}"/>
