 ...
 * <input type="text"   permission="'+permission+
 ...
 bind：key 为对话框返回字段
 	  val 为scope数据表达式
 */
formDirective.directive('htConselector',['$rootScope', function($rootScope) {
    return {
    	restrict : 'AE',
    	templateUrl:__ctx +'/weixin/bpm/conSelector.html',
    	scope: {
    		htConselector:'='
    	},
        link: function($scope, element, attrs) {
        	...
        	$scope.initData=function(){
        		for(var key in bind){
            		...
            		console.log("($scope.$parent." + bind[key] +")");
        	...
        	//对值进行双向绑定
        	$scope.$watch('htConselector',  function(newValue, oldValue) {
        	...
        	$scope.showDialog = function(){
        		...
        		//扩展参数   added by martic 2018年1月13日
        		if(conf.hasOwnProperty("dataqq")){
        			var data={};
        			var dataqq=conf.dataqq;
        			for(var i in dataqq){
        				var val=dataqq[i];
        				if(val.indexOf("data.main")>-1){
        					//将data.main.** 转换成对应的model的值
        					val=eval("($scope.$parent." + dataqq[i] +")")
        				}
        				data[i]=val;
        			}
        			conf.dataqq=data;
        	...
        	$scope.selectedOnClick = function(index){
        		...
        		if(selector.type =='UploadDialog'){
        			window.location.href=__ctx+"/weixin/system/sysFile/download.ht?fileId="+id;
        		}
			}
        ...
        link: function($scope, element, attrs) {
        	...
        	$scope.updScope =function(){
        		for(var key in bind){
        			...
        			var tmp='$scope.$parent.'+bind[key]+'="'+dataStr +'"';
        			eval(tmp);
        			//修改人员选择器人员id传到后台后无法匹配ID的问题  added by martic 2018年1月17日
        			if(bind[key].indexOf("id")>-1){        				
        				var tmp2='$scope.$parent.'+bind[key].replace("id","ID")+'="'+dataStr +'"';
        				eval(tmp2);
        			}
        		}
        		$scope.render();
...
.directive('htUpload',["$rootScope",function($rootScope) {
	return {
    	...
    	template : '<div ng-if="permission==\'w\' || permission==\'b\'">\
						<div class="custom-selector" ng-repeat="file in files" repeat-finish="renderFinish()"> 	\
							<button class="am-btn am-btn-secondary" ng-click="onClick(file.id)">${file.name}</button> \
							<a class="btn btn-xs  fa-remove"  title="移除该项" ng-click="remove($index)"></a> 	\
    					</div>				\
    					<a class="btn btn-sm btn-primary fa-upload" ng-click="showDialog()"></a>			\
    				</div>\
    				<div ng-if="permission==\'r\' || permission==\'rp\'">\
						<div ng-repeat="file in files" class="custom-selector custom-selector-r" >\
							<button class="am-btn am-btn-secondary" ng-click="onClick(file.id)">${file.name}</button>\
						</div>\
    				</div>',
        ...
        compile: function(element, attrs) {
			...
			if(permission=="w" || permission=="b"){
				...
				for(var i=0;i<valJson.length;i++){
	        		...
	        		jqEl.append('<input type="radio" id="'+id+'" name="" ng-model="'+attrs.htRadios+'"  value="'+jsonObj.val+'" /><label for="'+id+'">' + jsonObj.text + "</label>");
	        	}
			}
    ...
    return {
    	...
    	link:function(scope,element,attrs){
    		...
    		if(options){
    			...
    			if(format =='yyyy-MM-dd HH:mm:ss') preset = 'datetime';
    			if(format =='HH:mm:ss')preset = 'time';
    		}
    		...
    		if(permission=="w" || permission=="b"){
    			...
    			str='<div class="am-form-icon am-form-feedback">\
	    		     	<input id="" type="text"  ng-model="'+name+'" ht-validate="'+(attrs.htValidate?attrs.htValidate:"{}")+'"  permission="'+permission+'">	\
...
.directive('htValidate', [function () {
      return {
          ...
          link: function (scope, element, attr, ctrl) {
        	...
        	//made  by sammy-start
              var nameTemp=attr.ngModel;
              nameTemp=nameTemp.replace("data.","")
              //console.log(eval('scope.data.'+nameTemp));
              scope.$watch('permission.'+nameTemp,function(newValue,oldValue){
                  if(newValue!=oldValue){
                      
                      //alert("$scope."+nameTemp+"permission改变了："+newValue+"---"+oldValue);
                      attr.permission=newValue;
                      var validate = attr.htValidate;
                      var permission=attr.permission;
                      //如果不必填且没有其他校验返回
                      ...
                      //console.log('scope.data.'+nameTemp+"="+eval('scope.data.'+nameTemp));
                      if(permission=="b")validateJson.required = true;
                      var validity =  $.fn.validRules(eval('scope.data.'+nameTemp),validateJson,element);
