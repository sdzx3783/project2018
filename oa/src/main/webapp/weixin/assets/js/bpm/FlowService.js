...
flowServiceModule.factory('flowService',['baseService',function(baseService){
	return {
		 ...
		 * 获取发起表单界面
		 */
		getStartForm:function(defId,formKey,version,pk){
			var url=__ctx +"/weixin/bpm/getStartForm.ht";
			formKey=formKey || "";
			version=version || 0;
			pk=pk || "";
			var data={defId:defId,formKey:formKey,version:version,pk:pk};
		...
		handForm:function($scope,data,defId){
			if(!data.result){
				angular.element("#formHtml").html("出错了:" + data.msg);
				return;
			}
			var isGet=data.get;
			var temp="";
		...
		startFlow:function(flowKey,data,runId){
			var url=__ctx +"/weixin/bpm/task/startFlow.ht";
		...
		saveDraft:function(actDefId,data,runId,businessKey){
			var url=__ctx +"/weixin/bpm/task/saveForm.ht";
			if(businessKey && runId) url =__ctx +"/weixin/bpm/task/saveData.ht";
			var formJson=this.convertJson(data);
		...
		delDraft:function(runId){
			var url=__ctx +"/weixin/bpm/processRun/delDraft.ht";
		...
		approveFlow:function(taskId,voteAgree,voteContent,back,data){
			var url=__ctx +"/weixin/bpm/task/complete.ht";
			var formJson=this.convertJson(data);
			//解决附件提交时特殊符号双引号转义的问题
