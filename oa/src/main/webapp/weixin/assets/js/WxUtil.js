...
function createPopupDialog(conf,dialogId){
	...
	var aryDiv=['<div class="am-modal am-modal-no-btn" tabindex="-1" id="'+dialogId+'">',
	  '<div class="am-modal-dialog">',
	    ...
	    '</div>',
	    '<div class="am-modal-bd" style="width:100%;height:100%;-webkit-overflow-scrolling:touch;overflow-y:scroll;">',
	    	iframe ,
	    '</div>',
	  '</div>',
	'</div>']
 ...
 * @param dialogId dialog类型 eg:orgDialog,UserDialog
 */
function createPopupDialogConfirm(conf,dialogId,confirmCallback){
	var title=conf.title || "对话框";
	var iframeId ="dialogId_"+Math.random(1000);
	conf.iframeId = iframeId;
	...
	var aryDiv=['<div class="am-modal am-modal-no-btn" tabindex="-1" id="'+dialogId+'">',
	  '<div class="am-modal-dialog">',
	    '<div class="am-modal-hd">',
	      ...
	      '<a href="javascript: void(0)" class="am-close am-close-spin" data-am-modal-close>&times;</a>',
	    '</div>',
	    '<div class="am-modal-bd" style="padding: 0 10px;">',
	    	iframe ,
	    '</div>',
	    '<div class="am-modal-footer" style="display: table; border-top: 1px solid #dedede;">',
	      '<span class="am-modal-btn" data-am-modal-cancel>取消</span>',
	      '<span class="am-modal-btn am-modal-confirm-m">确定</span>',
	    '</div>',
	  '</div>',
	'</div>']
	...
	document.getElementById(iframeId).contentWindow.dialogConf=conf;
	popupDialog.modal({width:window.innerWidth,height:window.innerHeight});
	popupDialog.find('.am-modal-confirm-m').off('click.close.modal.amui').click(function() {
		confirmCallback();
	});
}
