...
PictureShowPlugin={
		...
		init:function(){
			...
			// pictureShow-div	最外层的容器
			// div_pictureShow	缩略图和大图的容器
			//	pictureShow_m_Entry_zp_Toolbar	上传和删除图片的容器
			//	wrap_div	大图的容器
			//	table_pictureShow	缩略图的容器
			...
			$(this.fileObjs).each(function(){
				...
				var html = '<div id="div_'+fn_+'" style="'+ $(this).attr("style")+'" class="pictureShow-div 我是最外层的容器">';
				html     += '<div id="div_'+fn_+'_container"></div>';
				html     += '<table id="pictureShow_'+fn_+'_Toolbar">';
				html     += '<tbody>';
				html     += '<tr>';
				html     += '<td>';
				html     += '<a href="javascript:;" field="'+fn+'" class="oa-button-label selectFile" atype="uploadPicture" onclick="{PictureShowPlugin.upLoadPictureFile(this);}">上传图片</a>';
				html     += '</td>';
				html     += '<td>';
				html     += '<a href="javascript:;" field="'+fn+'" class="oa-button-label del" atype="delPicture" onclick="{PictureShowPlugin.deletePictureFile(this);}">删除图片</a>';
				html     += '</td>';
				html     += '</tr>';
				html     += '</tbody>';
				html     += '</table>';
			...
			for ( var i = 0; i < this.fileObjs.length; i++) {
				...
				if(right=="no"){
					...
					//	这里获取了最外层容器的当前宽度，基于table的拉伸，
					//	是一个不确定的值，这里是图片控件的尺寸计算关键
					//	如果要将控件的尺寸固定，这里就不能动态获取，应该给
					//	一个固定值
					var  widthStr = '300px'; //div_rq.css("width");
					...
					div_rq.css("height","auto !important");		
