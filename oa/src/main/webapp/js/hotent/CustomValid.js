			...
			if(conf&&conf.returnErrorMsg){
				...
				if(!_v){
					...
					for(var m in msgs){
						...
						if(m=="_elem"){
							// 获取错误标题
							if($(msgs[m]).attr("name") && $(msgs[m]).attr("name").indexOf("s:")==0){
								//字表模板表单
								//如果获取title还是空值,那么可能是字表模板表单
								var $trTemp=$(msgs[m]).parents('td').parent("tr");
								var i=0;
								$trTemp.find("td:visible").each(function(){
									var isEqual=false;
								...
								errorTitle=$trTemp.siblings(".headRow:first").find("th:eq("+i+")","td:eq("+i+")").text();
							}
							if(errorTitle==""){
								//如果字表模板获取不到错误字段名称 或者 不是字表模板表单 通过此方法获取
