...
function BpmDefinitionDialog(conf) {
	...
	if (!conf.isSingle)
		conf.isSingle = false;
	var url = __ctx + '/makshi/dialog/bpmdefdialog/dialogDefs.ht';
	url = url.getNewUrl();
...
function vocationQualificationDialog(title,userId,registId,sealId){
	var that = this;
	var url = __ctx + '/makshi/title/vocationQualification/dialogVocation.ht?userId='+userId;
	if(registId != ''){
		 url += "&registNull=true";
	}
	if(sealId != ''){
...
//	var url = __ctx + '/makshi/dialog/bpmdefdialog/dialogDefs.ht';
	DialogUtil.open({
		...
		sucCall:function(rtn){
			if(registId != '')
				$(registId).val(rtn.regist_id);
			if(sealId != '')
				$(sealId).val(rtn.seal_id);
		}
	});
}
