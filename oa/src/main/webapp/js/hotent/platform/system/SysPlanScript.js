...
function editSysPlanExchange(id,planId){
    ...
    DialogUtil.open({
        ...
        isResize: false,
        onLoaded: function(){
            // 这里是main.jsp，接下来需要获取弹出的iframe标签，然后找到里面的
            // textarea元素，对其触发focus事件，
            // 以上是为了解决在ie下iframe嵌入页面表单元素无法聚焦的问题

            $(parent.document).find('.l-dialog.l-dialog-win iframe').contents().find('.content').focus();
        },
...
function setOwnerDiv(divId,arry,href,myId,myName,width,height,title){
    ...
    for(var i=0;i<arry.length;i++){
        ...
        ids += obj.id+",";
        names += obj.name+",";
        var span = $('<span class="oa-user-label"></span>').html(obj.name).attr("ownerId",obj.id);
    /*  if(href){
            a.attr("hrefstr",href+obj.id);
            a.attr("href","#");
        }*/
        if(width){
            span.attr("dialogWidth",width);
        }
        if(height){
            span.attr("dialogHeight",height);
        }
        if(title){
            span.attr("dialogTitle",title);
        }
        div.append(span);
    }
    ids = ids.substring(0, ids.length-1);
