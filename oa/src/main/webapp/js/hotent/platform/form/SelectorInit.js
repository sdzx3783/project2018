...
// var resetField = $('<a href="javascript:;" class="link reset" name=""
// >重置</a>');
var hiddenField = $('<input name="" type="hidden" lablename="" class="hidden" value="" >');

/**
 * 组建选择器 obj：将在此对象之前添加隐藏域，在其之后添加选择、重置按钮 className：选择器的class属性值
 */
...
/**
 * 显示选择器对话框。 obj 按钮控件 fieldName 字段名称 type :选择器类型。 1.人员选择器（单选） 2.人员选择器（多选）
 * 3.角色选择器（单选） 4.角色选择器（多选） 5.组织选择器（单选） 6.组织选择器（多选） 7.岗位选择器（单选） 8.岗位选择器（单选）
 */
function handSelector() {
	...
	// 10.流程实例选择器
	$("body").delegate("a.link.defs", "click", function() {
		selector($(this), 10);
	});
	// 重置选择器的值
...
function selector(obj, type) {
	...
	var fieldName = obj.attr("name"), parent = obj.parent(), idFilter = "input[name='" + fieldName + "ID']", nameFilter = "input[name='" + fieldName + "']", linkFielter = "a[name='"
			+ fieldName + "ID']", inputId = $(idFilter, parent), inputName = $(nameFilter, parent), link = $(linkFielter, parent), oldIdVal = inputId.val(), idStr = inputId.val(), nameStr = inputName
			.val(), scope = inputName.attr("scope"), arguments = [];
	var fieldValue = $("#" + obj.data('value')),fieldShow = $("#" + obj.data("show"));
	if (idStr) {
	...
	case 10:
		BpmDefinitionDialog({
			...
			callback : function(data) {
				...
				fieldValue.val(data.defId);
				fieldShow.html(data.defName);
			}
		});
	}
