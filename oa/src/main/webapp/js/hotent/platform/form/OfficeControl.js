...
OfficeControl=function(){
	...
	this.getTemplate=function(callback){
		var memo="overtime_application";
		var formKey=$("input[name='formKey']").val();
		//请假流程，查询年假
		if(formKey!=null && formKey!=''){
			memo=formKey;
		}
		var doctemplatesign=$("#doctemplatesign").val();
		
		if(doctemplatesign && doctemplatesign.length>0){
			memo=doctemplatesign;
		}
		var url=__ctx +"/platform/system/sysOfficeTemplate/dialog.ht?type="+this.templatetype+"&memo="+memo;
			...
			_self.getTemplate(function(rtn){
				...
				if(!(Sys.firefox) && !(Sys.chrome)){      //IE是同步的，它 会等待模版加载成功后执行书签的插入 （火狐和谷歌就不可以）
					...
					_self.controlObj.ActiveDocument.Bookmarks.Add(BookMarkName,saverange);
						...
						$(".insertContent").each(function(){
							    BookMarkName = $(this).attr("name");
							    var elObj = $(this).text();
							    if(_self.controlObj.ActiveDocument.BookMarks.Exists(BookMarkName))
								{
									bkmkObj = _self.controlObj.ActiveDocument.BookMarks(BookMarkName);	
									saverange = bkmkObj.Range;
									saverange.Text = elObj;
									_self.controlObj.ActiveDocument.Bookmarks.Add(BookMarkName,saverange);
								}
							  });
						$("[red-element]").each(function(){
							BookMarkName = $(this).attr("red-element");
							 var elObj = null;
							if($(this).is("td")){
								 elObj = $(this).text();
							}else{
...
function addTemplateOnComplete(myNum) {
	...
	var BookMarkName = "content";
	var bkmkObj = myObj.controlObj.ActiveDocument.BookMarks.Item(BookMarkName);	
	var saverange = bkmkObj.Range;
	saverange.Paste();
	myObj.controlObj.ActiveDocument.Bookmarks.Add(BookMarkName,saverange);
	
/*	$(".insertContent,.formInput").each(function(){
	    BookMarkName = $(this).attr("name");
	    var elObj = $(this).text();
	    if(myObj.controlObj.ActiveDocument.BookMarks.Exists(BookMarkName))
		{
			bkmkObj = myObj.controlObj.ActiveDocument.BookMarks(BookMarkName);	
			saverange = bkmkObj.Range;
			saverange.Text = elObj;
			myObj.controlObj.ActiveDocument.Bookmarks.Add(BookMarkName,saverange);
		}
	...
	$("[red-element]").each(function(){
	    ...
	    if(myObj.controlObj.ActiveDocument.BookMarks.Exists(BookMarkName))
		{			
			bkmkObj = myObj.controlObj.ActiveDocument.BookMarks.Item(BookMarkName);		
			saverange = bkmkObj.Range;
			saverange.Text = elObj;
			myObj.controlObj.ActiveDocument.Bookmarks.Add(BookMarkName,saverange);
		}
