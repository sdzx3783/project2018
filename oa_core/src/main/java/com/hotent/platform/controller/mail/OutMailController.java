...
import com.hotent.makshi.utils.EmojiUtil;
...
public class OutMailController extends BaseController
{
	...
	{	
		...
		if(BeanUtils.isEmpty(defaultMail)){
			...
			rstJson="{\"total\":"+count+",\"data\":"+resultMsg+"}";
			...
			filter.put("isRead", 0);
			List<OutMail> list=outMailService.getFolderList(queryFilter);
			count=queryFilter.getPageBean().getTotalCount();
			JSONArray json = JSONArray.fromObject(list); 
			resultMsg=json.toString();
			rstJson="{\"total\":"+count+",\"data\":"+resultMsg+"}";
			writeResultMessage(response.getWriter(),rstJson,ResultMessage.Success);
	...
	public ModelAndView edit(HttpServletRequest request) throws Exception
	{
		Long mailId=RequestUtil.getLong(request,"mailId");
		long userId=ContextUtil.getCurrentUserId();
		Integer rtype=0;//   0：不显示返回按钮 1：返回公司通讯录
		...
		//接收公司通讯录传值
		String receieveEmail = RequestUtil.getString(request, "receieveAddress");
		String receieveName=RequestUtil.getString(request, "receieveName");
		//receieveName = new String(receieveName.getBytes("ISO8859-1"), "UTF-8");
		OutMail outMail=null;
		boolean isadd=true;
		if(mailId==0){
			outMail=new OutMail();
			if(receieveEmail!=null && receieveEmail.length()>0){
				outMail.setReceiverAddresses(receieveEmail);
			}
		}else{
		...
		return getAutoView().addObject("outMail",outMail).addObject("returnUrl", returnUrl)
				.addObject("outMailUserSetingList",list).addObject("rtype", rtype).addObject("receieveName", receieveName);
	}
	...
	public void downLoadAttach(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		try {
			...
			if(sysFile!=null){// 发件箱中的附件，会在SysFile中有记录
				...
				String filePath = ServiceUtil.getMailAttachBasePath()+File.separator+sysFile.getFilePath();
				...
				FileUtil.downLoadFile(request, response, ServiceUtil.getMailAttachBasePath()+File.separator+filePath, entity.getFileName());
			}
		} catch (Exception e) {
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
