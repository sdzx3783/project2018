...
/*	@Resource
	...
	{	
		...
		List<SysOfficeTemplate> list=new ArrayList<>();
		Long userId =ContextUtil.getCurrentUserId();
		Integer templatetype=RequestUtil.getInt(request, "templatetype",1);
		filter.addFilter("templatetype", templatetype);
		ModelAndView mv=this.getAutoView();
		Object memo = filter.getFilters().get("memo");
		if(memo!=null && memo.toString().startsWith("doctemplatesign")){
			//文档模板模块
			String[] split = memo.toString().split("_");
			if(split.length>=2){
//				DocTemplate docTemplate = docTemplateService.getById(Long.parseLong(split[1]));
				/*if(docTemplate!=null){
					String file = docTemplate.getFile();
					if(file!=null && file.trim().length()>0){
						JSONArray jsonArray = JSONArray.fromObject(file);
						JSONObject jsonObject = jsonArray.getJSONObject(0);
						Long fileid = Long.parseLong(jsonObject.get("id").toString());
						SysFile sysFile = sysFileService.getById(fileid);
						if(sysFile!=null){
							
							SysOfficeTemplate sysOfficeTemplate=new SysOfficeTemplate();
							sysOfficeTemplate.setSubject(docTemplate.getName());
							sysOfficeTemplate.setPath(sysFile.getFilePath());
							sysOfficeTemplate.setId(docTemplate.getId());
							list.add(sysOfficeTemplate);
							filter.getPageBean().setTotalCount(list.size());
							
						}
					}
				}*/
			}
			mv.setViewName("/makshi/doc/docTemplateSelector.jsp");
			mv.addObject("pageBeansysOfficeTemplateItem", filter.getPageBean())// 此两项用于分页标签
			.addObject("requestURIsysOfficeTemplateItem", request.getRequestURI());
		}else{
			list=sysOfficeTemplateService.getOfficeTemplateByUserId(userId,filter);
		}
		
		//List<SysOfficeTemplate> list=sysOfficeTemplateService.getAll(filter);
		
		mv.addObject("sysOfficeTemplateList",list)
	...
	{
		return this.getAutoView().addObject("type", RequestUtil.getString(request, "type")).addObject("memo", RequestUtil.getString(request, "memo"));
	}
	...
	public void getTemplateById(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if(saveType.contains("database")){    
			...
			String filePath ="";
			if(sysOfficeTemplate==null){
				//如果为空 去文档模板表里查
				/*DocTemplate docTemplate = docTemplateService.getById(templateId);
				if(docTemplate!=null){
					String file = docTemplate.getFile();
					if(file!=null && file.trim().length()>0){
						JSONArray jsonArray = JSONArray.fromObject(file);
						...
						SysFile sysFile = sysFileService.getById(fileid);
						if(sysFile!=null){
							filePath =attachPath +File.separator + sysFile.getFilePath();
						}else{
							return ;
						}
					}else{
				...
				//系统模板表
				filePath =attachPath +File.separator + sysOfficeTemplate.getPath();	
			}
