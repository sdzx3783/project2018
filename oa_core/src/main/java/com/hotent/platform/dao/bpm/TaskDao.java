...
public class TaskDao extends BaseDao<TaskEntity>
{
	...
	public int getMyUndoneTasksCount(Long userId) {
		...
		List<Integer> list = getBySqlKeyGenericity("getAllMyTaskCount", params);

		return (list != null && list.size() > 0) ?  list.get(0).intValue() : 0;
	}
	
	public void updateAgentSetting(AgentSetting agentSetting, List<AgentDef> list) {
		Map<String,Object> params=new HashMap<String,Object>();
		List<String> defList = new ArrayList<String>();
		Long owner = agentSetting.getAuthid();
		Long agent = agentSetting.getAgentid();
		...
		if(authtype == AgentSetting.AUTHTYPE_GENERAL){
			this.update("updateAgentSettingAll",params);
		}else{
			if(null!=list && list.size()>0){
				for(AgentDef agentDef:list){
					String flowkey = agentDef.getFlowkey();
					defList.add(flowkey);
				}
			}
			params.put("defList",defList);
			this.update("updateAgentSetting",params);
		}
	}
}
