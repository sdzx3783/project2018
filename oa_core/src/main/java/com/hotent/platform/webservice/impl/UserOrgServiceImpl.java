...
public class UserOrgServiceImpl implements UserOrgService {
		...
		try{
			...
			if (demens.size() > 0) {
				for (int i = 0; i < demens.size(); i++) {
					resultList.addAll(getChilderOrg(demens.get(i).getDemId()));
				}
			}
	...
	public String getSysOrgListByOrgTactic() {
		JSONArray jsonArray = JSONArray.fromObject(sysOrgTacticService.getSysOrgListByOrgTactic());
