...
public class FlowServiceImpl implements FlowService {
     ...
     */
    @Override
    public String alreadyCompletedMattersList(String account) {
        BaseRuelt result = new BaseRuelt();
        QueryFilter filter = new QueryFilter(request);
        filter.getFilters().clear();
        String currentPage = request.getParameter("pageNo") != null ? request.getParameter("pageNo") : "1";
        String pageSize = request.getParameter("pageSize") != null ? request.getParameter("pageSize") : "15";
        ...
        SysUser user = sysUserService.getByAccount(account);
        if (BeanUtils.isEmpty(user)) {
            throw new ApiExcetpion(ReturnCode.NO_DATA_FOUND,"该账号的用户不存在");
        }
        ContextUtil.setCurrentUser((ISysUser) user);
        filter.setPageBean(pb);
        filter.addFilter("assignee", ContextUtil.getCurrentUserId().toString());// 用户id
        filter.addFilter("allowMobile", "1");// 用户id
     ...
     * 获取指定account账户的待办事宜
     */
    public String getMyTaskByAccount(String account){
        BaseRuelt result = new BaseRuelt();
 
            //传入的账户
/*//查询条件         
 			String accountstr = request.getParameter("account");
            String taskNodeName = request.getParameter("taskNodeName");
            ...
            if(StringUtils.isNotBlank(subjectName)){
            	queryFilter.addFilter("subject", subjectName);
            }
            //获取手机表单
            List processTasks= taskDao.getMyMobileTasks(user.getUserId(), queryFilter);
//            List<ProcessTask> processTasks = taskDao.getMyMobileTasks(user.getUserId(), queryFilter);
            ...
            Map<String, Object> map = new HashMap<String, Object>();
            if("onlyCount".equals(onlyCount)){
            	map.put("count", queryFilter.getPageBean().getTotalCount());
            }else {				
            	map.put("list", processTasks);
			}
            result.setResultMap(map, ReturnCode.SUCCESS);
/*            result.put("totalCount", ((PageList<ProcessTask>) processTasks).getTotalCount());
     ...
     */
    @Override
    public String getMyCompletedListJson(String account) {
        /*JSONObject result = new JSONObject();
        try {
            JSONObject jsonObject = JSONObject.fromObject(json);
            String classType = jsonObject.get("classType") != null ? jsonObject.getString("classType") : "";
...
/*                List<Map<String, Object>> mapList = new ArrayList<Map<String, Object>>();
    ...
    @Override
    public String getMyFlowListJson(String account) {
        BaseRuelt result = new BaseRuelt();
//        request
//        HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest();
        QueryFilter filter = new QueryFilter(request);
        account=request.getParameter("account");
        if (StringUtils.isEmpty(account)) {// 没传用户就报错
            throw new ApiExcetpion(ReturnCode.PRAMS_IS_NULL,"必须传入用户账号(account)");
        }
        SysUser user = sysUserService.getByAccount(account);
        if (BeanUtils.isEmpty(user))
        ...
        String inOrNotStr=request.getParameter("inOrNot");
        ...
        if (StringUtil.isNotEmpty(classType)) {
            ...
            boolean inOrNotBool = inOrNotStr!= null ? Boolean.getBoolean(inOrNotStr) : true;// 包含还是排除\
            if (inOrNotBool) {
                filter.addFilter("nodeKey", classType);
            } else {
                filter.addFilter("notNodeKey", classType);
        ...
        map.put("list", GsonUtil.toJsonTree(mapList).toString());
        result.setResultMap(map, ReturnCode.SUCCESS);
        return result.getResultMap().toString();
    }
 
    /**
     * 获取指定UserAccount用户我申请的流程
     */
    @Override
    public String getMyRequestListJson(String account) {
        BaseRuelt result = new BaseRuelt();
/*        JSONObject result = new JSONObject();
        try {
            JSONObject jsonObject = JSONObject.fromObject(json);*/
...
//                throw new Exception("必须传入用户账号(account)");
                throw new ApiExcetpion(ReturnCode.PRAMS_IS_NULL);
            SysUser user = sysUserService.getByAccount(account);
            if (BeanUtils.isEmpty(user))
//                throw new Exception("该账号的用户不存在");
                throw new ApiExcetpion(ReturnCode.PRAMS_IS_NULL);
            ...
            String currentPage=request.getParameter("pageNo")==null?"1":request.getParameter("pageNo");
            PageBean pb = filter.getPageBean();
            if (StringUtil.isNotEmpty(pageSize)) {
                pb.setPagesize(Integer.parseInt(pageSize));
            }
            if (StringUtil.isNotEmpty(currentPage)) {
...
/*            if (StringUtil.isNotEmpty(classType)) {
                ...
                boolean inOrNot = request.getParameter("inOrNot") != null ? request.getParameter("inOrNot") : true;// 包含还是排除\
                if (inOrNot) {
                    filter.addFilter("nodeKey", classType);
                } else {
                    filter.addFilter("notNodeKey", classType);
