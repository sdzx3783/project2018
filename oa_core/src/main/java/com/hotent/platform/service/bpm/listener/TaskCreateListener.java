...
public class TaskCreateListener extends BaseTaskListener {
	...
	private void handAssignUserFromDb(String actDefId,String nodeId,DelegateTask delegateTask){
		...
		ExecutionStackService executionStackService=(ExecutionStackService) AppUtil.getBean(ExecutionStackService.class);
		...
		if(processInstance!=null){
			...
			//add by sherwin 处理转办找回上一步的原始执行人
			List<ExecutionStack> processExecution = executionStackService.getByActInstId(Long.parseLong(actInstId));
			Long parentId = 0L; 
			int depth = 0;
			for (ExecutionStack executionStack : processExecution) {
				if (executionStack.getDepth()>=depth) {
					depth = executionStack.getDepth();
					parentId = executionStack.getStackId();
				}
			}		
			ExecutionStack executionStack = executionStackService.getById(parentId);	
			List<TaskOpinion> opinionList = taskOpinionService.getByActInstIdTaskKey(Long.parseLong(actInstId), executionStack.getNodeId());
			if (executionStack == null || opinionList == null || opinionList.size() <= 1) {
				...
				TaskOpinion opinion = null;
				Collections.sort(opinionList,new Comparator(){

					@Override
					public int compare(Object o1, Object o2) {
						if (((TaskOpinion)o1).getCheckStatus()>((TaskOpinion)o2).getCheckStatus()) {
							return -1;
						}else
							return 1;
				...
				for (TaskOpinion taskOpinion : opinionList) {
					if (opinion == null || ((taskOpinion.getCheckStatus() ==TaskOpinion.STATUS_DELEGATE||taskOpinion.getCheckStatus() ==TaskOpinion.STATUS_AGENT) && taskOpinion.getStartTime().getTime() <opinion.getStartTime().getTime() )) {
						opinion = taskOpinion;
					}
				}
				//获取上个任务的执行人，这个执行人在上一个流程任务的完成事件中进行设置。
				...
				String preStepUserId=opinion.getExeUserId().toString();
				Long preStepOrgId=sysOrgService.getOrgIdByUserId(Long.parseLong(preStepUserId));
				vars.put(BpmConst.PRE_ORG_ID, preStepOrgId);
