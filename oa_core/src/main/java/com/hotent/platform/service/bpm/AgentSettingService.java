...
public class AgentSettingService extends BaseService<AgentSetting>
{
	...
	public void delAll(Long[] lAryId) {
		for(Long id:lAryId){
			//删除代理，恢复待办任务执行人
			AgentSetting agentSetting =  dao.getById(id);
			List<AgentDef> list = getAgentDefList(id);
			taskDao.updateAgentSetting(agentSetting,list);
			delByPk(id);
			dao.delById(id);	
		}	
	}
	...
	public void updateAll(AgentSetting agentSetting) throws Exception{
		if(AgentSetting.ENABLED_NO.toString().equals(agentSetting.getEnabled().toString())){
