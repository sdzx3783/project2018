...
public class OutMailService extends BaseService<OutMail>
{
    ...
    private void saveAttach(Part message, Mail mail, String userAccount)throws Exception{
    	...
    	String mailAttachBasePath = ServiceUtil.getMailAttachBasePath();
    	...
    	String relateFilePath=userAccount+File.separator+year+File.separator+month+File.separator+UniqueIdUtil.genId()+"."+FileUtil.getFileExt(filename);
    	String filePath=mailAttachBasePath + File.separator + relateFilePath;
			...
			if(jsonArray.size()>0){
				...
				for(Object obj:jsonArray){
					...
					if(PictureHandlerController.PICFILE_TYPEKEY.equals(sysFile.getFileType())){
						basePath = ServiceUtil.getPictureBasePath();
					}
