	...
	private void updateStatus(ProcessCmd cmd, ProcessRun processRun) {
		...
		case 1:
			if (isRecover)
				...
				//合同盖章流程 驳回需要回收合同编号 
				/*if(processRun.getDefId()==10000001600044l &&
						StringUtils.isNotEmpty(cmd.getDestTask()) && "UserTask7".equals(cmd.getDestTask())){
	...
	public ResultMessage redo(Long runId, String informType, String memo) throws Exception {
		...
		try {
			...
			if(10000009830004l==defId){
				...
				/*LeaveApplication leaveApplication = leaveApplicationService.getById(Long.valueOf(processRun.getBusinessKey()));
				if(leaveApplication!=null && "年假".equals(leaveApplication.getLeave_type())){
					String leave_days = leaveApplication.getLeave_days();
					
					String sql1="UPDATE w_anuual_leave set hasleaveCurrentyear=hasleaveCurrentyear-"+Double.valueOf(leave_days).doubleValue()
							+",utime=NOW() where userid="+processRun.getCreatorId();
	...
	private List<ProcessTask> getLastTaskId(Long actInstId,ResultMessage resultMsg){
		...
		for(ExecutionStack stack:stackList){
			...
			if (stack.getTaskIds().equals(taskEntity.getId())) {
				if (starcki != 0) {
					if (stackList.get(starcki - 1).getNodeId()
							.contains("SignTask")) {
						String[] tasks = stackList.get(starcki - 1)
								.getTaskIds().split(",");
						List<ProcessTask> signTaskList = new ArrayList<ProcessTask>();
						for (ProcessTask task : taskList) {
							if (task.getTaskDefinitionKey().contains(
									stackList.get(starcki - 1).getNodeId())) {
	...
	public String getFormDetailByRunId(Long runId, String ctxPath) throws Exception {
		...
		if (processRun.getFormDefId() == 0L && StringUtil.isNotEmpty(processRun.getBusinessUrl())) {
			...
			if(BeanUtils.isZeroEmpty(formDefId)){
				BpmFormDef formDef =bpmFormDefService.getGlobalFormDef(processRun.getActDefId());
				if(BeanUtils.isNotEmpty(formDef)){
					formDefId = formDef.getFormDefId();
				}
			}
