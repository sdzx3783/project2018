...
public class SysUserOrgService extends BaseService<UserPosition>
{
	 ...
	 * @param args
	 */
	public List<SysOrg> getAllOrgIdByOrgId(Long orgId){
		List<SysOrg> rtnList=new ArrayList<SysOrg>();
		if(orgId!=null && !("").equals(orgId)){
				List<SysOrg> allList=new ArrayList<SysOrg>();
				allList = getAllChildrenOrgIds(orgId);
				if(allList!= null && allList.size()>0){
					rtnList.addAll(allList);
			}
		}
		return rtnList;
	}
	
	public List<SysOrg> getAllChildrenOrgIds(Long orgId){
		List<SysOrg> allList=new ArrayList<SysOrg>();
		List<SysOrg> allListTemp=new ArrayList<SysOrg>();
		allList = getChildrenOrgIds(orgId);
		if(allList!= null && allList.size()>0){
			allListTemp.addAll(allList);
			for(SysOrg sysOrg:allList){
				List<SysOrg> allChildrenList= getAllChildrenOrgIds(sysOrg.getOrgId());
				if(allChildrenList != null && allChildrenList.size()>0){
					allListTemp.addAll(allChildrenList);
				}
			}
		}
		return allListTemp;
	...
	public List<SysOrg> getChildrenOrgIds(Long orgId){
		return sysOrgDao.getAllOrgIdByOrgId(orgId);
	}
	public static void main(String[] args) {
		Long orgId = 10000007780600L;
	//	getAllOrgIdByOrgId(orgId)
