...
import com.hotent.platform.model.worktime.WorkItem;
...
public class BpmBatchApprovalService extends BaseService<BpmBatchApproval> {
	...
	private List<Map<String, Object>> getDataList(String defKey, String nodeId,
		...
		logger.debug(sql);
		//考勤填报批量处理
		if("gstbsq".equals(defKey) && dataList.size()>0){
			String sqlids="''";
			for (String businessKey : businessKeyList) {
				sqlids+="'"+businessKey+"'"+",";
			}
			if(sqlids.length()>2){
				sqlids=sqlids.substring(2, sqlids.length()-1);
			}
			Map<String,Object> param=new HashMap<>();
			param.put("refids", sqlids);
			List<WorkItem> wks=new ArrayList<>();
			List bySqlKey = workTimeDao.getBySqlKey("getWorkItemsByIds", param);
			if(bySqlKey!=null){
				wks =bySqlKey;
			}
			for (WorkItem wk : wks) {
				String refid = wk.getRefid();
				for (Map<String, Object> object : dataList) {
					Object id = object.get("id");
					if(refid.equals(id+"")){
						Object rm=object.get("remarkData");
						if(rm==null){
							List<WorkItem> workItems=new ArrayList<>();
							workItems.add(wk);
							object.put("remarkData", workItems);
						}else{
							List<WorkItem> workItems=(List<WorkItem>) rm;
							workItems.add(wk);
						}
					}
				}
	...
	private String transformSql(List<String> fieldList,
		...
		String temp="";
		if(tableName.equalsIgnoreCase("W_work_hour_application")){
			temp=",W_work_hour_application.ID as id";//特殊处理考勤填报 多返回一个字段id
		}
		pkField = tableName + "." + pkField;
		sbf.append(
				" select task.ID_ as actTaskId ,task.DESCRIPTION_ AS actDescription , run.SUBJECT as actSubject, ")
				.append(fields).append(temp)
				.append(" from ")
				.append(tableName)
				.append(" left join bpm_pro_run run on run.businessKey = ")
				.append(pkField)
				.append(" left join act_ru_task task on task.proc_inst_id_=run.actInstId and task.ASSIGNEE_=").append(ContextUtil.getCurrentUserId())
