...
import com.hotent.platform.service.form.parser.FieldAbstractParser;
import com.hotent.platform.service.form.parser.BpmFormDefHtmlParser;
import com.hotent.platform.service.form.parser.ParserInter;
...
public abstract class TableAbstractParser extends FieldAbstractParser {
	...
	public String getValue(Element element, BpmFormData data, String fieldName, boolean isDelSubId, String initVal) {
		...
		if (ename.startsWith("m:")) {//主表
			val = StringUtil.toString(data.getMainField(fieldName.toLowerCase()));
		} else {//子表
			SubTable subTable = data.getSubTableMap().get(tableName.toLowerCase());
			String subid = element.attr(BpmFormDefHtmlParser.SUBID);//获取其是不是子表循环复制出来的数据
			if (StringUtil.isEmpty(subid)) {
				val = StringUtil.toString(subTable.getRow().get(fieldName.toLowerCase()));
			} else {
