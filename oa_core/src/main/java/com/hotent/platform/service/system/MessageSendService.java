...
public class MessageSendService extends BaseService<MessageSend>
{
	 ...
	 * @return
	 */
	public List<MessageSend> getMyMessage(long userId,QueryFilter queryFilter){
		Map<String,Object> params = new HashMap<String,Object>();
		params.put("userId", userId);
		return dao.getBySqlKey("getMyMessage",params, queryFilter.getPageBean());
	}
	 ...
	 * @return
	 */
	public List<MessageSend> notReadMsglist(QueryFilter queryFilter){
		return dao.notReadMsglist(queryFilter);
	}
	 ...
	 * @return
	 */
	public List<MessageSend> notReadMsglistDialog(QueryFilter queryFilter){
		return dao.notReadMsglistDialog(queryFilter);
	}
	 ...
	 * @return
	 */
	public List<MessageSend> getMessageReply(long msgId){
		Map<String,Object> params = new HashMap<String,Object>();
		params.put("msgId", msgId);
		return dao.getBySqlKey("getMessageReply",params);
	}
		...
		throws Exception
	{
		...
		if (receiverId.length() > 0){
			for (int i = 0; i < idArr.length; i++){
				...
				if(Long.parseLong(idArr[i])!=0L){
					appInnerMessageService.addMyMessage("0",null,Long.parseLong(idArr[i]),null,null,messageSend.getContentText(),"{\"url\":\"oaapp_messagelist:\"}","新的站内消息","站内消息");
				}
			}
		}
		...
		if (receiverOrgId.length() > 0){
			for (int i = 0; i < orgIdArr.length; i++){
				...
				if (StringUtil.isNotEmpty(orgIdArr[i])){
					...
					appInnerMessageService.addMyMessage("1",Long.parseLong(orgIdArr[i]),null,null,null,messageSend.getContentText(),"{\"url\":\"oaapp_messagelist:\"}","新的站内消息","站内消息");
				}
				messageReceiverDao.add(messageReceiver);
			}
		}
