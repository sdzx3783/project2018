...
public class MessageLogService extends BaseService<MessageLog> {
	 ...
	 * @param state 状态
	 * @param msgSendId
	 * @param msgToMobStatus
	 */
	public void addSMSLog(String subject, String receiver,
							  Integer messageType, Integer state, String msgSendId, Integer msgToMobStatus) {
		MessageLog messageLog = new MessageLog();
		messageLog.setId(UniqueIdUtil.genId());
		messageLog.setSubject(StringUtils.substring(subject, 0, 50));
		messageLog.setReceiver(receiver);
		...
		messageLog.setMsgSendId(msgSendId);
		messageLog.setMsgToMobStatus(msgToMobStatus);
		dao.add(messageLog);
	}
}
