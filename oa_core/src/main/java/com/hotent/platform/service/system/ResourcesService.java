...
public class ResourcesService extends BaseService<Resources>
{
	 ...
	 * @return
	 */
	public List<Resources> getSysMenu(SubSystem sys,SysUser user,Long resOrgId){
		...
		if(ContextUtil.isSuperAdmin()){
			...
			resourcesList=resourcesDao.getSuperMenuByAllRoleAndOrg(systemId,resOrgId);
		}else{
			if(StringUtil.isNotEmpty(rolealias)){
//				resourcesList=resourcesDao.getNormMenuByAllRole(systemId,rolealias);
				resourcesList=resourcesDao.getNormMenuByAllRoleAndOrg(systemId,rolealias,resOrgId);
			}else{
				//resourcesList=resourcesDao.getNormMenuByRole(systemId,user.getUserId());
				resourcesList=resourcesDao.getNormMenuByRoleAndOrg(systemId,user.getUserId(),resOrgId);
			}
		}
