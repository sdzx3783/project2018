...
public class GroovyCondition implements Condition {
	...
	private static final String GROOVYCONFFILENAME="groovyadjust";
	...
	public boolean evaluate(DelegateExecution execution) {
		...
		//特殊处理请假流程手机和web表单分支条件大小写不一致问题
		Properties properties = PropUtils.getProperties(GROOVYCONFFILENAME);
		Set<Object> keySet = properties.keySet();
		for (Object object : keySet) {
			if(object!=null){
				String obj=(String) object;
				String val = properties.getProperty(obj);
				Object realObj = maps.get(obj);
				if(realObj!=null && !("").equals(realObj)){
					if(StringUtils.isNotEmpty(obj)  && StringUtils.isNotEmpty(val)
							&& maps.containsKey(obj)){
						maps.put(val, maps.get(obj));
					}
				}
			}
		}
		/*if(maps.containsKey("needactual")){
			maps.put("needActual", maps.get("needactual"));
		}
		if(maps.containsKey("needcompanyleader")){
			maps.put("needCompanyLeader", maps.get("needcompanyleader"));
		}*/
